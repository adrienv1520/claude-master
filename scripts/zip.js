/* eslint-disable unicorn/prevent-abbreviations,security-node/detect-crlf */
...
const directoriesToZip = args.length > 0
  ? args
  : allDirectories;
...
for (const directory of directoriesToZip) {
  ...
  try {
    execSync(`zip -r ${zipDirectoryPath} ${directoryPath}`, { stdio: 'inherit' });
    success.push(name);
