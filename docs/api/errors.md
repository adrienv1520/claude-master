...
When receiving a [streaming](../developer/build-with-claude-streaming.md) response via SSE, it's possible that an error can occur after returning a 200 response, in which case error handling wouldn't follow these standard mechanisms.
...
In accordance with our [versioning](./versioning.md) policy, we may expand the values within these objects, and it is possible that the `type` values will grow over time.
...
<Warning>
 We highly encourage using the [streaming Messages API](../developer/build-with-claude-streaming.md) or [Message Batches API](https://platform.claude.com/docs/en/api/creating-message-batches.md) for long running requests, especially those over 10 minutes.
</Warning>

We do not recommend setting a large `max_tokens` values without using our [streaming Messages API](../developer/build-with-claude-streaming.md)
