...
When receiving a [streaming](../developer/developer-build-with-claude-streaming.md) response via SSE, it's possible that an error can occur after returning a 200 response, in which case error handling wouldn't follow these standard mechanisms.
...
<Warning>
 We highly encourage using the [streaming Messages API](../developer/developer-build-with-claude-streaming.md) or [Message Batches API](https://platform.claude.com/docs/en/api/creating-message-batches.md) for long running requests, especially those over 10 minutes.
</Warning>

We do not recommend setting a large `max_tokens` values without using our [streaming Messages API](../developer/developer-build-with-claude-streaming.md)
or [Message Batches API](https://platform.claude.com/docs/en/api/creating-message-batches.md):
...
- Networks differ in reliability; our [Message Batches API](https://platform.claude.com/docs/en/api/creating-message-batches.md) can help you
...
See [Streaming Messages](../developer/developer-build-with-claude-streaming.md#get-the-final-message-without-handling-events) for more details.
...
Use [structured outputs](../developer/developer-build-with-claude-structured-outputs.md), system prompt instructions, or [`output_config.format`](../developer/developer-build-with-claude-structured-outputs.md#json-outputs) instead.