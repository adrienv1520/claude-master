...
Requests in this tier are prioritized over all other requests to Anthropic. This prioritization helps minimize ["server overloaded" errors](./errors.md#http-errors), even during peak times.
...
- For [long-context](../developer/build-with-claude-context-windows.md) (>200k input tokens) requests, input tokens are 2 tokens per token
- All other input tokens are 1 token per token

**Output Tokens**
- For [long-context](../developer/build-with-claude-context-windows.md) (>200k input tokens) requests, output tokens are 1.5 tokens per token
...
Check the [model overview page](../developer/about-claude-models-overview.md) for more details on our models.
