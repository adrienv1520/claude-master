...
[Structured Outputs](../developer/developer-build-with-claude-structured-outputs.md) (beta) ensures that the model generates responses that adhere to a supplied JSON schema.
...
Structured Outputs supports a [subset of the JSON Schema language](../developer/developer-build-with-claude-structured-outputs.md#json-schema-limitations). Schemas are generated automatically from classes to align with this subset. The method `outputConfig(Class<T>)` performs a validation check on the schema derived from the specified class.
...
[Tool Use](../developer/developer-agents-and-tools-tool-use-overview.md) lets you integrate external tools and functions directly into the AI model's responses. You define JSON schemas for tools, and the model uses the schemas to decide when and how to use these tools.
...
The SDK provides support for the [Message Batches API](../developer/developer-build-with-claude-batch-processing.md) under the `client.messages().batches()` namespace. See the [pagination section](#pagination) for how to iterate through batch results.
...
This can be used to quickly log failing requests and report them back to Anthropic. For more information on debugging requests, see the [API error documentation](./api-errors.md#request-id).
...
- [Amazon Bedrock](../developer/developer-build-with-claude-claude-on-amazon-bedrock.md)
...
You can access most beta API features through the `beta()` method on the client. To check the availability of all of Claude's capabilities and tools, see the [build with Claude overview](../developer/developer-build-with-claude-overview.md).

For example, to use the [Files API](../developer/developer-build-with-claude-files.md):
...
- [Streaming guide](../developer/developer-build-with-claude-streaming.md)
- [Tool use guide](../developer/developer-agents-and-tools-tool-use-overview.md)