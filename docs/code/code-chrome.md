...
Claude Code integrates with the Claude in Chrome browser extension to give you browser automation capabilities from the CLI or the [VS Code extension](./code-vs-code.md#automate-browser-tasks-with-chrome). Build your code, then test and debug in the browser without switching contexts.
...
* [Claude Code](./code-quickstart.md#step-1-install-claude-code) version 2.0.73 or higher
...
For VS Code, see [browser automation in VS Code](./code-vs-code.md#automate-browser-tasks-with-chrome).

### Enable Chrome by default

To avoid passing `--chrome` each session, run `/chrome` and select "Enabled by default".

In the [VS Code extension](./code-vs-code.md#automate-browser-tasks-with-chrome), Chrome is available whenever the Chrome extension is installed. No additional flag is needed.
...
* [Computer use](./code-computer-use.md): control native macOS apps when a task can't be done in a browser
* [Use Claude Code in VS Code](./code-vs-code.md#automate-browser-tasks-with-chrome): browser automation in the VS Code extension
* [CLI reference](./code-cli-reference.md): command-line flags including `--chrome`
* [Common workflows](./code-common-workflows.md): more ways to use Claude Code
* [Data and privacy](./code-data-usage.md): how Claude Code handles your data
