...
Plan Mode instructs Claude to create a plan by analyzing the codebase with read-only operations, perfect for exploring codebases, planning complex changes, or reviewing code safely. In Plan Mode, Claude uses [`AskUserQuestion`](./code-settings.md#tools-available-to-claude) to gather requirements and clarify your goals before proposing a plan.
...
For more control over the process, guide Claude through it step-by-step or [create your own skill](./code-skills.md):
...
| ---------------------- | ------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Effort level**       | Adjust in `/model` or set [`CLAUDE_CODE_EFFORT_LEVEL`](./code-settings.md#environment-variables) | Control thinking depth for Opus 4.6: low, medium, high (default). See [Adjust effort level](./code-model-config.md#adjust-effort-level)                     |
...
| **Limit token budget** | Set [`MAX_THINKING_TOKENS`](./code-settings.md#environment-variables) environment variable       | Limit the thinking budget to a specific number of tokens (ignored on Opus 4.6 unless set to 0). Example: `export MAX_THINKING_TOKENS=10000`           |
...
**With Opus 4.6**, thinking uses adaptive reasoning: the model dynamically allocates thinking tokens based on the [effort level](./code-model-config.md#adjust-effort-level) you select (low, medium, high). This is the recommended way to tune the tradeoff between speed and reasoning depth.

**With other models**, thinking uses a fixed budget of up to 31,999 tokens from your output budget. You can limit this with the [`MAX_THINKING_TOKENS`](./code-settings.md#environment-variables) environment variable, or disable thinking entirely via `/config` or the `Option+T`/`Alt+T` toggle.
...
For the full walkthrough with JSON configuration examples, see [Automate workflows with hooks](./code-hooks-guide.md#get-notified-when-claude-needs-input). For the complete event schema and notification types, see the [Notification reference](./code-hooks.md#notification).
