...
[Plugins](./plugins.md) can provide custom subagents that integrate seamlessly with Claude Code. Plugin agents work identically to user-defined agents and appear in the `/agents` interface.
...
See the [plugin components reference](./plugins-reference.md#agents) for details on creating plugin agents.
...
For detailed information about the JSON format and all available options, see the [CLI reference documentation](./cli-reference.md#agents-flag-format).
...
| `model`          | No       | Model to use for this subagent. Can be a model alias (`sonnet`, `opus`, `haiku`) or `'inherit'` to use the main conversation's model. If omitted, defaults to the [configured subagent model](./model-config.md) |
...
The `model` field allows you to control which [AI model](./model-config.md) the subagent uses:
...
Subagents can be granted access to any of Claude Code's internal tools. See the [tools documentation](./settings.md#tools-available-to-claude) for a complete list of available tools.
...
* [Plugins](./plugins.md) - Extend Claude Code with custom agents through plugins
* [Slash commands](./slash-commands.md) - Learn about other built-in commands
* [Settings](./settings.md) - Configure Claude Code behavior
