...
This page covers how computer use works in the CLI. For the Desktop app on macOS or Windows, see [computer use in Desktop](./code-desktop.md#let-claude-use-your-computer).
...
* If you have an [MCP server](./code-mcp.md) for the service, Claude uses that.
...
Claude's level of control also varies by app category: browsers and trading platforms are view-only, terminals and IDEs are click-only, and everything else gets full control. See [app permissions in Desktop](./code-desktop.md#app-permissions) for the complete tier breakdown.
...
* You're on macOS. Computer use in the CLI is not available on Linux or Windows. On Windows, use [computer use in Desktop](./code-desktop.md#let-claude-use-your-computer) instead.
...
* [Computer use in Desktop](./code-desktop.md#let-claude-use-your-computer): the same capability with a graphical settings page
* [Claude in Chrome](./code-chrome.md): browser automation for web-based tasks
* [MCP](./code-mcp.md): connect Claude to structured tools and APIs
* [Sandboxing](./code-sandboxing.md): how Claude's Bash tool isolates filesystem and network access
