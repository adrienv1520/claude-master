...
* [Claude for Teams or Enterprise](./code-setup.md#for-teams-and-organizations) (recommended)
* [Claude Console with team billing](./code-setup.md#for-teams-and-organizations)
...
4. Each invited user needs to complete these steps:
   ...
   * [Check system requirements](./code-setup.md#system-requirements)
   * [Install Claude Code](./code-setup.md#installation)
...
1. Follow the [Bedrock docs](./code-amazon-bedrock.md), [Vertex docs](./code-google-vertex-ai.md), or [Microsoft Foundry docs](./code-microsoft-foundry.md)
2. Distribute the environment variables and instructions for generating cloud credentials to your users. Read more about how to [manage configuration here](./code-settings.md).
3. Users can [install Claude Code](./code-setup.md#installation)
...
<Note>
  For a quick reference on permission rule syntax including wildcards, see [Permission rule syntax](./code-settings.md#permission-rule-syntax) in the settings documentation.
</Note>

#### Permission modes

Claude Code supports several permission modes that can be set as the `defaultMode` in [settings files](./code-settings.md#settings-files):
...
| `dontAsk`           | Auto-denies tools unless pre-approved via `/permissions` or [`permissions.allow`](./code-settings.md#permission-settings) rules |
...
Use `Task(AgentName)` rules to control which [subagents](./code-sub-agents.md) Claude can use:

* `Task(Explore)` Matches the Explore subagent
* `Task(Plan)` Matches the Plan subagent
* `Task(Verify)` Matches the Verify subagent

Add these rules to the `deny` array in your [settings](./code-settings.md#permission-settings) or use the `--disallowedTools` CLI flag to disable specific agents. For example, to disable the Explore agent:
...
[Claude Code hooks](./code-hooks-guide.md) provide a way to register custom shell commands to perform permission evaluation at runtime. When Claude Code makes a tool call, PreToolUse hooks run before the permission system runs, and the hook output can determine whether to approve or deny the tool call in place of the permission system.

### Managed settings

For organizations that need centralized control over Claude Code configuration, administrators can deploy `managed-settings.json` files to [system directories](./code-settings.md#settings-files). These policy files follow the same format as regular settings files and cannot be overridden by user or project settings.
...
* **Custom credential scripts**: The [`apiKeyHelper`](./code-settings.md#available-settings) setting can be configured to run a shell script that returns an API key.
