...
For organizations that prefer file-based policy distribution, Claude Code also supports `managed-settings.json` files that can be deployed to [system directories](./code-settings.md#settings-files). These policy files follow the same format as regular settings files and cannot be overridden by user or project settings.
