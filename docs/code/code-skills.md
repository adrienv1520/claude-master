...
<Note>
  For built-in commands like `/help` and `/compact`, see [interactive mode](./code-interactive-mode.md#built-in-commands).
...
<Note>
  CLAUDE.md files from `--add-dir` directories are not loaded by default. To load them, set `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD=1`. See [Load memory from additional directories](./code-memory.md#load-memory-from-additional-directories).
</Note>
