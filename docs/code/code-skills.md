...
| Plugin     | Bundled with [plugins](./code-plugins.md)              | Anyone with the plugin installed  |
...
| **[Slash commands](./code-slash-commands.md)** | Create reusable prompts (e.g., `/deploy staging`)                          | You type `/command` to run it              |
| **[CLAUDE.md](./code-memory.md)**              | Set project-wide instructions (e.g., "use TypeScript strict mode")         | Loaded into every conversation             |
| **[Subagents](./code-sub-agents.md)**          | Delegate tasks to a separate context with its own tools                    | Claude delegates, or you invoke explicitly |
| **[Hooks](./code-hooks.md)**                   | Run scripts on events (e.g., lint on file save)                            | Fires on specific tool events              |
| **[MCP servers](./code-mcp.md)**               | Connect Claude to external tools and data sources                          | Claude calls MCP tools as needed           |
...
| `agent`          | No       | Specify which [agent type](./code-sub-agents.md#built-in-subagents) to use when `context: fork` is set (e.g., `Explore`, `Plan`, `general-purpose`, or a custom agent name from `.claude/agents/`). Defaults to `general-purpose` if not specified. Only applicable when combined with `context: fork`. |
| `hooks`          | No       | Define hooks scoped to this Skill's lifecycle. Supports `PreToolUse`, `PostToolUse`, and `Stop` events.                                                                                                                                                                                           |
| `user-invocable` | No       | Controls whether the Skill appears in the slash command menu. Does not affect the [`Skill` tool](./code-slash-commands.md#skill-tool) or automatic discovery. Defaults to `true`. See [Control Skill visibility](#control-skill-visibility).                                                            |
...
2. **Programmatic invocation**: Claude calls it via the [`Skill` tool](./code-slash-commands.md#skill-tool)
...
[Subagents](./code-sub-agents.md) do not automatically inherit Skills from the main conversation. To give a custom subagent access to specific Skills, list them in the subagent's `skills` field:
...
* **Plugins**: To share Skills across multiple repositories, create a `skills/` directory in your [plugin](./code-plugins.md) with Skill folders containing `SKILL.md` files. Distribute through a [plugin marketplace](./code-plugin-marketplaces.md).
* **Managed**: Administrators can deploy Skills organization-wide through [managed settings](./code-iam.md#managed-settings). See [Where Skills live](#where-skills-live) for managed Skill paths.
