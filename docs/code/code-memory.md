...
Subagents can also maintain their own auto memory. See [subagent configuration](./code-sub-agents.md#enable-persistent-memory) for details.
...
<Note>
  Rules load into context every session or when matching files are opened. For task-specific instructions that don't need to be in context all the time, use [skills](./code-skills.md) instead, which only load when you invoke them or when Claude determines they're relevant to your prompt.
</Note>
...
<Steps>
  ...
  <Step title="Deploy with your configuration management system">
    Use MDM, Group Policy, Ansible, or similar tools to distribute the file across developer machines. See [managed settings](./code-permissions.md#managed-settings) for other organization-wide configuration options.
  </Step>
</Steps>

A managed CLAUDE.md and [managed settings](./code-settings.md#settings-files) serve different purposes. Use settings for technical enforcement and CLAUDE.md for behavioral guidance:
...
Patterns are matched against absolute file paths using glob syntax. You can configure `claudeMdExcludes` at any [settings layer](./code-settings.md#settings-files): user, project, local, or managed policy. Arrays merge across layers.
...
For instructions you want at the system prompt level, use [`--append-system-prompt`](./code-cli-reference.md#system-prompt-flags). This must be passed every invocation, so it's better suited to scripts and automation than interactive use.

<Tip>
  Use the [`InstructionsLoaded` hook](./code-hooks.md#instructionsloaded) to log exactly which instruction files are loaded, when they load, and why. This is useful for debugging path-specific rules or lazy-loaded files in subdirectories.
</Tip>
...
* [Skills](./code-skills.md): package repeatable workflows that load on demand
* [Settings](./code-settings.md): configure Claude Code behavior with settings files
* [Manage sessions](./code-sessions.md): manage context, resume conversations, and run parallel sessions
* [Subagent memory](./code-sub-agents.md#enable-persistent-memory): let subagents maintain their own auto memory
