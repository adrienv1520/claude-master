...
<Note>
  Code Review is in research preview, available for [Teams and Enterprise](https://claude.ai/admin-settings/claude-code) subscriptions. It is not available for organizations with [Zero Data Retention](./code-zero-data-retention.md) enabled.
</Note>
...
To run Claude in your own CI infrastructure instead of this managed service, see [GitHub Actions](./code-github-actions.md) or [GitLab CI/CD](./code-gitlab-ci-cd.md). For repositories on a self-hosted GitHub instance, see [GitHub Enterprise Server](./code-github-enterprise-server.md).
...
<Steps>
  ...
  <Step title="Install the Claude GitHub App">
    ...
    * **Pull requests**: read and write
    ...
    Code Review uses read access to contents and write access to pull requests. The broader permission set also supports [GitHub Actions](./code-github-actions.md) if you enable that later.
  </Step>
...
Claude reads `CLAUDE.md` files at every level of your directory hierarchy, so rules in a subdirectory's `CLAUDE.md` apply only to files under that path. See the [memory documentation](./code-memory.md) for more on how `CLAUDE.md` works.
...
* [Analytics](./code-analytics.md): track Claude Code usage beyond code review
