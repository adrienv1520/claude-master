...
|                            | [Cloud](./code-web-scheduled-tasks.md) | [Desktop](./code-desktop-scheduled-tasks.md) | [`/loop`](./code-scheduled-tasks.md) |
| :------------------------- | :------------------------------- | :------------------------------------- | :----------------------------- |
...
* **Desktop app**: open the **Schedule** page, click **New task**, and choose **New remote task**. See [Desktop scheduled tasks](./code-desktop-scheduled-tasks.md) for details.
...
<Steps>
  ...
  <Step title="Select an environment">
    Select a [cloud environment](./code-claude-code-on-the-web.md#cloud-environment) for the task. Environments control what the cloud session has access to:
    ...
    * **Setup script**: run install commands before each session starts, like installing dependencies or configuring tools
    ...
    A **Default** environment is available out of the box. To use a custom environment, [create one](./code-claude-code-on-the-web.md#cloud-environment) before creating the task.
  </Step>
  ...
  <Step title="Review connectors">
    All of your connected [MCP connectors](./code-mcp.md) are included by default. Remove any that the task doesn't need. Connectors give Claude access to external services like Slack, Linear, or Google Drive during each run.
  </Step>
...
Each task runs in a [cloud environment](./code-claude-code-on-the-web.md#cloud-environment) that controls network access, environment variables, and setup scripts. Configure environments before creating a task to give Claude access to APIs, install dependencies, or restrict network scope. See [cloud environment](./code-claude-code-on-the-web.md#cloud-environment) for the full setup guide.
...
* [Desktop scheduled tasks](./code-desktop-scheduled-tasks.md): schedule tasks that run on your machine with access to local files. The Desktop app's **Schedule** page shows both local and remote tasks in the same grid.
* [`/loop` and CLI scheduled tasks](./code-scheduled-tasks.md): lightweight scheduling within a CLI session
* [Cloud environment](./code-claude-code-on-the-web.md#cloud-environment): configure the runtime environment for cloud tasks
* [MCP connectors](./code-mcp.md): connect external services like Slack, Linear, and Google Drive
* [GitHub Actions](./code-github-actions.md): run Claude in your CI pipeline on repo events
