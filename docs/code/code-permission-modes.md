...
Modes set the baseline. Layer [permission rules](./code-permissions.md#manage-permissions) on top to pre-approve or block specific tools in any mode except `bypassPermissions`, which skips the permission layer entirely.
...
<Tabs>
  <Tab title="CLI">
    ...
    **As a default**: set `defaultMode` in [settings](./code-settings.md#settings-files).
    ...
    The same `--permission-mode` flag works with `-p` for [non-interactive runs](./code-headless.md).
  </Tab>
  ...
  <Tab title="Desktop">
    Use the mode selector next to the send button. Auto and Bypass permissions appear only after you enable them in Desktop settings. See the [Desktop guide](./code-desktop.md#choose-a-permission-mode).
  </Tab>
...
* Refine with [Ultraplan](./code-ultraplan.md) for browser-based review
...
* **Admin**: on Team and Enterprise, an admin must enable it in [Claude Code admin settings](https://claude.ai/admin-settings/claude-code) before users can turn it on. Admins can also lock it off by setting `permissions.disableAutoMode` to `"disable"` in [managed settings](./code-permissions.md#managed-settings).
...
The classifier trusts your working directory and your repo's configured remotes. Everything else is treated as external until you [configure trusted infrastructure](./code-permissions.md#configure-the-auto-mode-classifier).
...
Run `claude auto-mode defaults` to see the full rule lists. If routine actions get blocked, an administrator can add trusted repos, buckets, and services via the `autoMode.environment` setting: see [Configure the auto mode classifier](./code-permissions.md#configure-the-auto-mode-classifier).
...
In [non-interactive mode](./code-headless.md) with the `-p` flag, repeated blocks abort the session since there is no user to prompt.

Repeated blocks usually mean the classifier is missing context about your infrastructure. Use `/feedback` to report false positives, or have an administrator [configure trusted infrastructure](./code-permissions.md#configure-the-auto-mode-classifier).
...
<AccordionGroup>
  <Accordion title="How the classifier evaluates actions">
    ...
    1. Actions matching your [allow or deny rules](./code-permissions.md#manage-permissions) resolve immediately
  ...
  <Accordion title="How auto mode handles subagents">
    The classifier checks [subagent](./code-sub-agents.md) work at three points:
...
* [Permissions](./code-permissions.md): allow, ask, and deny rules; auto mode classifier configuration; managed policies
* [Hooks](./code-hooks.md): custom permission logic via `PreToolUse` and `PermissionRequest` hooks
* [Ultraplan](./code-ultraplan.md): run plan mode in a Claude Code on the web session with browser-based review
* [Security](./code-security.md): safeguards and best practices
* [Sandboxing](./code-sandboxing.md): filesystem and network isolation for Bash commands
* [Non-interactive mode](./code-headless.md): run Claude Code with the `-p` flag
