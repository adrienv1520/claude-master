...
<Tip>
  For a quickstart guide with examples, see [Get started with Claude Code hooks](./code-hooks-guide.md).
</Tip>

## Configuration

Claude Code hooks are configured in your [settings files](./code-settings.md):
...
[Plugins](./code-plugins.md) can provide hooks that integrate seamlessly with your user and project hooks. Plugin hooks are automatically merged with your configuration when plugins are enabled.
...
In addition to settings files and plugins, hooks can be defined directly in [Skills](./code-skills.md), [subagents](./code-sub-agents.md), and [slash commands](./code-slash-commands.md) using frontmatter. These hooks are scoped to the component's lifecycle and only run when that component is active.
...
* `Task` - Subagent tasks (see [subagents documentation](./code-sub-agents.md))
* `Bash` - Shell commands
...
[Model Context Protocol (MCP) tools](./code-mcp.md). When MCP servers
...
<Tip>
  For practical examples including code formatting, notifications, and file protection, see [More Examples](./code-hooks-guide.md#more-examples) in the get started guide.
</Tip>
