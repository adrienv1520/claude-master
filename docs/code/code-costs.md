...
MCP tool definitions are [deferred by default](./code-mcp.md#scale-with-mcp-tool-search), so only tool names enter context until Claude uses a specific tool. Run `/context` to see what's consuming space.
