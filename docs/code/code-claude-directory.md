...
| ----------------------- | -------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `managed-settings.json` | System-level, varies by OS | Enterprise-enforced settings that you can't override. See [server-managed settings](./code-server-managed-settings.md).     |
...
<Note>
  ...
  * [Managed settings](./code-server-managed-settings.md) deployed by your organization take precedence over everything
  * CLI flags like `--permission-mode` or `--settings` override `settings.json` for that session
  * Some environment variables take precedence over their equivalent setting, but this varies: check the [environment variables reference](./code-env-vars.md) for each one

  See [settings precedence](./code-settings.md#settings-precedence) for the full order.
</Note>
...
* [Manage Claude's memory](./code-memory.md): write and organize CLAUDE.md, rules, and auto memory
* [Configure settings](./code-settings.md): set permissions, hooks, environment variables, and model defaults
* [Create skills](./code-skills.md): build reusable prompts and workflows
* [Configure subagents](./code-sub-agents.md): define specialized agents with their own context
