...
This guide covers the core architecture, built-in capabilities, and [tips for working effectively](#work-effectively-with-claude-code). For step-by-step walkthroughs, see [Common workflows](./code-common-workflows.md). For extensibility features like skills, MCP, and hooks, see [Extend Claude Code](./code-features-overview.md).
...
[Multiple models](./code-model-config.md) are available with different tradeoffs. Sonnet handles most coding tasks well. Opus provides stronger reasoning for complex architectural decisions. Switch with `/model` during a session or start with `claude --model <name>`.
...
| **Code intelligence** | See type errors and warnings after edits, jump to definitions, find references (requires [code intelligence plugins](./code-discover-plugins.md#code-intelligence)) |

These are the primary capabilities. Claude also has tools for spawning subagents, asking you questions, and other orchestration tasks. See [Tools available to Claude](./code-settings.md#tools-available-to-claude) for the complete list.
...
**Extending the base capabilities:** The built-in tools are the foundation. You can extend what Claude knows with [skills](./code-skills.md), connect to external services with [MCP](./code-mcp.md), automate workflows with [hooks](./code-hooks.md), and offload tasks to [subagents](./code-sub-agents.md). These extensions form a layer on top of the core agentic loop. See [Extend Claude Code](./code-features-overview.md) for guidance on choosing the right extension for your needs.

## What Claude can access

This guide focuses on the terminal. Claude Code also runs in [VS Code, JetBrains IDEs, and other environments](./code-ide-integrations.md).
...
**Sessions are independent.** Each new session starts with a fresh context window, without the conversation history from previous sessions. Claude can persist learnings across sessions using [auto memory](./code-memory.md#auto-memory), and you can add your own persistent instructions in [CLAUDE.md](./code-memory.md).
...
Since sessions are tied to directories, you can run parallel Claude sessions by using [git worktrees](./code-common-workflows.md#run-parallel-claude-code-sessions-with-git-worktrees), which create separate directories for individual branches.
...
Claude's context window holds your conversation history, file contents, command outputs, [CLAUDE.md](./code-memory.md), loaded skills, and system instructions. As you work, context fills up. Claude compacts automatically, but instructions from early in the conversation can get lost. Put persistent rules in CLAUDE.md, and run `/context` to see what's using space.
...
[Skills](./code-skills.md) load on demand. Claude sees skill descriptions at session start, but the full content only loads when a skill is used. For skills you invoke manually, set `disable-model-invocation: true` to keep descriptions out of context until you need them.

[Subagents](./code-sub-agents.md) get their own fresh context, completely separate from your main conversation. Their work doesn't bloat your context. When done, they return a summary. This isolation is why subagents help with long sessions.

See [context costs](./code-features-overview.md#understand-context-costs) for what each feature costs, and [reduce token usage](./code-costs.md#reduce-token-usage) for tips on managing context.
...
* **Delegate mode**: Claude coordinates work through [agent teammates](./code-agent-teams.md) only, with no direct implementation. Only available when an agent team is active.

You can also allow specific commands in `.claude/settings.json` so Claude doesn't ask each time. This is useful for trusted commands like `npm test` or `git status`. Settings can be scoped from organization-wide policies down to personal preferences. See [Permissions](./code-permissions.md) for details.
