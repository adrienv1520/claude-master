...
* **Before you type anything**: CLAUDE.md, auto memory, MCP tool names, and skill descriptions all load into context. Your own setup may add more here, like an [output style](./code-output-styles.md) or text from [`--append-system-prompt`](./code-cli-reference.md), which both go into the system prompt the same way.
* **As Claude works**: each file read adds to context, [path-scoped rules](./code-memory.md#path-specific-rules) load automatically alongside matching files, and a [PostToolUse hook](./code-hooks-guide.md) fires after each edit.
* **The follow-up prompt**: a [subagent](./code-sub-agents.md) handles the research in its own separate context window, so the large file reads stay out of yours. Only the summary and a small metadata trailer come back.
* **At the end**: `/compact` replaces the conversation with a structured summary. Most startup content reloads automatically. The [skill](./code-skills.md) listing is the one exception.
...
* [Extend Claude Code](./code-features-overview.md): when to use CLAUDE.md vs skills vs rules vs hooks vs MCP
