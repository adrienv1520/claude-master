...
Claude Code [passes hook input as JSON](./code-hooks.md#pretooluse-input) via stdin to hook commands. The validation script reads this JSON, extracts the Bash command, and [exits with code 2](./code-hooks.md#exit-code-2-behavior) to block write operations:
...
See [Hook input](./code-hooks.md#pretooluse-input) for the complete input schema and [exit codes](./code-hooks.md#simple-exit-code) for how exit codes affect behavior.
...
Claude Code [passes hook input as JSON](./code-hooks.md#pretooluse-input) via stdin to hook commands. The validation script reads this JSON, extracts the command being executed, and checks it against a list of SQL write operations. If a write operation is detected, the script [exits with code 2](./code-hooks.md#exit-code-2-behavior) to block execution and returns an error message to Claude via stderr.
...
The hook receives JSON via stdin with the Bash command in `tool_input.command`. Exit code 2 blocks the operation and feeds the error message back to Claude. See [Hooks](./code-hooks.md#simple-exit-code) for details on exit codes and [Hook input](./code-hooks.md#pretooluse-input) for the complete input schema.
