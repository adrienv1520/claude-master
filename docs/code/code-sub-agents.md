...
See [Hook input](./code-hooks.md#pretooluse-input) for the complete input schema and [exit codes](./code-hooks.md#exit-codes) for how exit codes affect behavior.
...
For programmatic usage, see [Subagents in the Agent SDK](./code-agent-sdk/subagents.md).
...
The hook receives JSON via stdin with the Bash command in `tool_input.command`. Exit code 2 blocks the operation and feeds the error message back to Claude. See [Hooks](./code-hooks.md#exit-codes) for details on exit codes and [Hook input](./code-hooks.md#pretooluse-input) for the complete input schema.
