...
[Plugins](./code-plugins.md) can provide custom subagents that integrate seamlessly with Claude Code. Plugin agents work identically to user-defined agents and appear in the `/agents` interface.
...
See the [plugin components reference](./code-plugins-reference.md#agents) for details on creating plugin agents.
...
For detailed information about the JSON format and all available options, see the [CLI reference documentation](./code-cli-reference.md#agents-flag-format).
...
| `model`          | No       | Model to use for this subagent. Can be a model alias (`sonnet`, `opus`, `haiku`) or `'inherit'` to use the main conversation's model. If omitted, defaults to the [configured subagent model](./code-model-config.md) |
...
The `model` field allows you to control which [AI model](./code-model-config.md) the subagent uses:
...
Subagents can be granted access to any of Claude Code's internal tools. See the [tools documentation](./code-settings.md#tools-available-to-claude) for a complete list of available tools.
...
You can disable specific built-in or custom subagents using the `Task(AgentName)` permission rule syntax. Add these rules to the `deny` array in your [settings](./code-settings.md#permission-settings) or use the `--disallowedTools` CLI flag.
...
* [Plugins](./code-plugins.md) - Extend Claude Code with custom agents through plugins
* [Slash commands](./code-slash-commands.md) - Learn about other built-in commands
* [Settings](./code-settings.md) - Configure Claude Code behavior
* [Hooks](./code-hooks.md) - Automate workflows with event handlers
