...
<Note>
  Summarize keeps you in the same session and compresses context. If you want to branch off and try a different approach while preserving the original session intact, use [fork](./code-how-claude-code-works.md#resume-or-fork-sessions) instead (`claude --continue --fork-session`).
</Note>
...
* [Built-in commands](./code-commands.md) - Accessing checkpoints using `/rewind`
