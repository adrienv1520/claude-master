...
After [installing Claude Code](./code-setup.md#install-claude-code), run `claude` in your terminal. On first launch, Claude Code opens a browser window for you to log in.
...
* **Cloud providers**: if your organization uses [Amazon Bedrock](./code-amazon-bedrock.md), [Google Vertex AI](./code-google-vertex-ai.md), or [Microsoft Foundry](./code-microsoft-foundry.md), set the required environment variables before running `claude`. No browser login is needed.
...
<Steps>
  ...
  <Step title="Users complete setup">
    ...
    * [Install Claude Code](./code-setup.md#install-claude-code)
...
<Steps>
  <Step title="Follow provider setup">
    Follow the [Bedrock docs](./code-amazon-bedrock.md), [Vertex docs](./code-google-vertex-ai.md), or [Microsoft Foundry docs](./code-microsoft-foundry.md).
  </Step>

  <Step title="Distribute configuration">
    Distribute the environment variables and instructions for generating cloud credentials to your users. Read more about how to [manage configuration here](./code-settings.md).
  </Step>

  <Step title="Install Claude Code">
    Users can [install Claude Code](./code-setup.md#install-claude-code).
  </Step>
</Steps>
...
1. Cloud provider credentials, when `CLAUDE_CODE_USE_BEDROCK`, `CLAUDE_CODE_USE_VERTEX`, or `CLAUDE_CODE_USE_FOUNDRY` is set. See [third-party integrations](./code-third-party-integrations.md) for setup.
2. `ANTHROPIC_AUTH_TOKEN` environment variable. Sent as the `Authorization: Bearer` header. Use this when routing through an [LLM gateway or proxy](./code-llm-gateway.md) that authenticates with bearer tokens rather than Anthropic API keys.
...
4. [`apiKeyHelper`](./code-settings.md#available-settings) script output. Use this for dynamic or rotating credentials, such as short-lived tokens fetched from a vault.
...
[Claude Code on the Web](./code-claude-code-on-the-web.md) always uses your subscription credentials. `ANTHROPIC_API_KEY` and `ANTHROPIC_AUTH_TOKEN` in the sandbox environment do not override them.
