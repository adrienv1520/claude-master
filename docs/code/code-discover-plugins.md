...
Looking to create and distribute your own marketplace? See [Create and distribute a plugin marketplace](./code-plugin-marketplaces.md).
...
<Note>
  The official marketplace is maintained by Anthropic. To distribute your own plugins, [create your own marketplace](./code-plugin-marketplaces.md) and share it with users.
</Note>
...
You can also [create your own LSP plugin](./code-plugins-reference.md#lsp-servers) for other languages.
...
These plugins bundle pre-configured [MCP servers](./code-mcp.md) so you can connect Claude to external services without manual setup:
...
<Steps>
  ...
  <Step title="Install a plugin">
    ...
    * **Local scope**: install for yourself in this repository only
    ...
    See [Configuration scopes](./code-settings.md#configuration-scopes) to learn more about scopes.
  </Step>
...
<Note>
  URL-based marketplaces have some limitations compared to Git-based marketplaces. If you encounter "path not found" errors when installing plugins, see [Troubleshooting](./code-plugin-marketplaces.md#plugins-with-relative-paths-fail-in-url-based-marketplaces).
</Note>
...
To choose a different [installation scope](./code-settings.md#configuration-scopes), use the interactive UI: run `/plugin`, go to the **Discover** tab, and press **Enter** on a plugin. You'll see options for:
...
You may also see plugins with **managed** scope—these are installed by administrators via [managed settings](./code-settings.md#settings-files) and cannot be modified.
...
To disable all automatic updates entirely for both Claude Code and all plugins, set the `DISABLE_AUTOUPDATER` environment variable. See [Auto updates](./code-setup.md#auto-updates) for details.
...
For full configuration options including `extraKnownMarketplaces` and `enabledPlugins`, see [Plugin settings](./code-settings.md#plugin-settings).
...
2. **Update Claude Code**:
   ...
   * **Native installer**: Re-run the install command from [Setup](./code-setup.md)
...
* **Plugin Skills not appearing**: Clear the cache with `rm -rf ~/.claude/plugins/cache`, restart Claude Code, and reinstall the plugin. See [Plugin Skills not appearing](./code-skills.md#plugin-skills-not-appearing-after-installation) for details.

For detailed troubleshooting with solutions, see [Troubleshooting](./code-plugin-marketplaces.md#troubleshooting) in the marketplace guide. For debugging tools, see [Debugging and development tools](./code-plugins-reference.md#debugging-and-development-tools).

## Next steps

* **Build your own plugins**: See [Plugins](./code-plugins.md) to create custom commands, agents, and hooks
* **Create a marketplace**: See [Create a plugin marketplace](./code-plugin-marketplaces.md) to distribute plugins to your team or community
* **Technical reference**: See [Plugins reference](./code-plugins-reference.md) for complete specifications
