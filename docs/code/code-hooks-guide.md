...
For other ways to extend Claude Code, see [skills](./code-skills.md) for giving Claude additional instructions and executable commands, [subagents](./code-sub-agents.md) for running tasks in isolated contexts, and [plugins](./code-plugins.md) for packaging extensions to share across projects.

<Tip>
  This guide covers common use cases and how to get started. For full event schemas, JSON input/output formats, and advanced features like async hooks and MCP tool hooks, see the [Hooks reference](./code-hooks.md).
</Tip>
...
Hooks let you run code at key points in Claude Code's lifecycle: format files after edits, block commands before they execute, send notifications when Claude needs input, inject context at session start, and more. For the full list of hook events, see the [Hooks reference](./code-hooks.md#hook-lifecycle).
...
You can replace the `echo` with any command that produces dynamic output, like `git log --oneline -5` to show recent commits. For injecting context on every session start, consider using [CLAUDE.md](./code-memory.md) instead. For environment variables, see [`CLAUDE_ENV_FILE`](./code-hooks.md#persist-environment-variables) in the reference.
...
The matcher filters by configuration type: `user_settings`, `project_settings`, `local_settings`, `policy_settings`, or `skills`. To block a change from taking effect, exit with code 2 or return `{"decision": "block"}`. See the [ConfigChange reference](./code-hooks.md#configchange) for the full input schema.
...
See the [CwdChanged](./code-hooks.md#cwdchanged) and [FileChanged](./code-hooks.md#filechanged) reference entries for input schemas, `watchPaths` output, and `CLAUDE_ENV_FILE` details.
...
Keep the matcher as narrow as possible. Matching on `.*` or leaving the matcher empty would auto-approve every permission prompt, including file writes and shell commands. See the [PermissionRequest reference](./code-hooks.md#permissionrequest-decision-control) for the full set of decision fields.
...
| `TeammateIdle`       | When an [agent team](./code-agent-teams.md) teammate is about to go idle                                                                                     |
...
Your script can parse that JSON and act on any of those fields. `UserPromptSubmit` hooks get the `prompt` text instead, `SessionStart` hooks get the `source` (startup, resume, clear, compact), and so on. See [Common input fields](./code-hooks.md#common-input-fields) in the reference for shared fields, and each event's section for event-specific schemas.
...
A fourth value, `"defer"`, is available in [non-interactive mode](./code-headless.md) with the `-p` flag. It exits the process with the tool call preserved so an Agent SDK wrapper can collect input and resume. See [Defer a tool call for later](./code-hooks.md#defer-a-tool-call-for-later) in the reference.

Returning `"allow"` skips the interactive prompt but does not override [permission rules](./code-permissions.md#manage-permissions). If a deny rule matches the tool call, the call is blocked even when your hook returns `"allow"`. If an ask rule matches, the user is still prompted. This means deny rules from any settings scope, including [managed settings](./code-settings.md#settings-files), always take precedence over hook approvals.

Other events use different decision patterns. For example, `PostToolUse` and `Stop` hooks use a top-level `decision: "block"` field, while `PermissionRequest` uses `hookSpecificOutput.decision.behavior`. See the [summary table](./code-hooks.md#decision-control) in the reference for a full breakdown by event.
...
For full matcher syntax, see the [Hooks reference](./code-hooks.md#configuration).
...
The `if` field uses [permission rule syntax](./code-permissions.md) to filter hooks by tool name and arguments together, so the hook process only spawns when the tool call matches. This goes beyond `matcher`, which filters at the group level by tool name only.
...
| [Skill](./code-skills.md) or [agent](./code-sub-agents.md) frontmatter | While the skill or agent is active | Yes, defined in the component file |

Run [`/hooks`](./code-hooks.md#the-hooks-menu) in Claude Code to browse all configured hooks grouped by event. To disable all hooks at once, set `"disableAllHooks": true` in your settings file.
...
For full configuration options, see [Prompt-based hooks](./code-hooks.md#prompt-based-hooks) in the reference.
...
For full configuration options, see [Agent-based hooks](./code-hooks.md#agent-based-hooks) in the reference.
...
The endpoint should return a JSON response body using the same [output format](./code-hooks.md#json-output) as command hooks. To block a tool call, return a 2xx response with the appropriate `hookSpecificOutput` fields. HTTP status codes alone cannot block actions.
...
For full configuration options and response handling, see [HTTP hooks](./code-hooks.md#http-hook-fields) in the reference.
...
* `PermissionRequest` hooks do not fire in [non-interactive mode](./code-headless.md) (`-p`). Use `PreToolUse` hooks for automated permission decisions.
* `Stop` hooks fire whenever Claude finishes responding, not only at task completion. They do not fire on user interrupts. API errors fire [StopFailure](./code-hooks.md#stopfailure) instead.
* When multiple PreToolUse hooks return [`updatedInput`](./code-hooks.md#pretooluse) to rewrite a tool's arguments, the last one to finish wins. Since hooks run in parallel, the order is non-deterministic. Avoid having more than one hook modify the same tool's input.
...
* [Hooks reference](./code-hooks.md): full event schemas, JSON output format, async hooks, and MCP tool hooks
* [Security considerations](./code-hooks.md#security-considerations): review before deploying hooks in shared or production environments
