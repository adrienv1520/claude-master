...
Voice dictation streams your recorded audio to Anthropic's servers for transcription. Audio is not processed locally. The speech-to-text service is only available when you authenticate with a Claude.ai account, and is not available when Claude Code is configured to use an Anthropic API key directly, Amazon Bedrock, Google Vertex AI, or Microsoft Foundry. See [data usage](./code-data-usage.md) for how Anthropic handles your data.

Voice dictation also needs local microphone access, so it does not work in remote environments such as [Claude Code on the web](./code-claude-code-on-the-web.md) or SSH sessions. In WSL, voice dictation requires WSLg for audio access, which is included with WSL2 on Windows 11. On Windows 10 or WSL1, run Claude Code in native Windows instead.
...
Voice dictation persists across sessions. Run `/voice` again to turn it off, or set it directly in your [user settings file](./code-settings.md):
...
While voice dictation is enabled, the input footer shows a `hold Space to speak` hint when the prompt is empty. The hint does not appear if you have a [custom status line](./code-statusline.md) configured.
...
Voice dictation uses the same [`language` setting](./code-settings.md) that controls Claude's response language. If that setting is empty, dictation defaults to English.
...
The push-to-talk key is bound to `voice:pushToTalk` in the `Chat` context and defaults to `Space`. Rebind it in [`~/.claude/keybindings.json`](./code-keybindings.md):
...
* [Customize keyboard shortcuts](./code-keybindings.md): rebind `voice:pushToTalk` and other CLI keyboard actions
* [Configure settings](./code-settings.md): full reference for `voiceEnabled`, `language`, and other settings keys
* [Interactive mode](./code-interactive-mode.md): keyboard shortcuts, input modes, and session controls
* [Built-in commands](./code-commands.md): reference for `/voice`, `/config`, and all other commands
