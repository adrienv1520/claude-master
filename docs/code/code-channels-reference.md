...
<Note>
  Channels are in [research preview](./code-channels.md#research-preview) and require Claude Code v2.1.80 or later. They require claude.ai login. Console and API key authentication is not supported. Team and Enterprise organizations must [explicitly enable them](./code-channels.md#enterprise-controls).
</Note>
...
To use an existing channel instead of building one, see [Channels](./code-channels.md). Telegram, Discord, iMessage, and fakechat are included in the research preview.
...
During the research preview, custom channels aren't on the [approved allowlist](./code-channels.md#supported-channels). Use `--dangerously-load-development-channels` to test locally. See [Test during the research preview](#test-during-the-research-preview) for details.
...
<Steps>
  ...
  <Step title="Test it">
    ...
    If you see "blocked by org policy," your Team or Enterprise admin needs to [enable channels](./code-channels.md#enterprise-controls) first.
...
During the research preview, every channel must be on the [approved allowlist](./code-channels.md#research-preview) to register. The development flag bypasses the allowlist for specific entries after a confirmation prompt. This example shows both entry types:
...
To make your channel installable and shareable, wrap it in a [plugin](./code-plugins.md) and publish it to a [marketplace](./code-plugin-marketplaces.md). Users install it with `/plugin install`, then enable it per session with `--channels plugin:<name>@<marketplace>`.

A channel published to your own marketplace still needs `--dangerously-load-development-channels` to run, since it isn't on the [approved allowlist](./code-channels.md#supported-channels). To get it added, [submit it to the official marketplace](./code-plugins.md#submit-your-plugin-to-the-official-marketplace). Channel plugins go through security review before being approved. On Team and Enterprise plans, an admin can instead include your plugin in the organization's own [`allowedChannelPlugins`](./code-channels.md#restrict-which-channel-plugins-can-run) list, which replaces the default Anthropic allowlist.

## See also

* [Channels](./code-channels.md) to install and use Telegram, Discord, iMessage, or the fakechat demo, and to enable channels for a Team or Enterprise org
...
* [MCP](./code-mcp.md) for the underlying protocol that channel servers implement
* [Plugins](./code-plugins.md) to package your channel so users can install it with `/plugin install`
