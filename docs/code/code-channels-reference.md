...
A channel published to your own marketplace still needs `--dangerously-load-development-channels` to run, since it isn't on the [approved allowlist](./code-channels.md#supported-channels). To get it added, [submit it to the official marketplace](./code-plugins.md#submit-your-plugin-to-the-official-marketplace). Channel plugins go through security review before being approved.
