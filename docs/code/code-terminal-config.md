...
For additional customization of the Claude Code interface itself, you can configure a [custom status line](./code-statusline.md) to display contextual information like the current model, working directory, or git branch at the bottom of your terminal.
...
If you see flicker during long sessions, or your terminal scroll position jumps to the top while Claude is working, try [fullscreen rendering](./code-fullscreen.md). It uses an alternate rendering path that keeps memory flat and adds mouse support. Enable it with `CLAUDE_CODE_NO_FLICKER=1`.
...
Claude Code supports a subset of Vim keybindings that can be enabled via `/config` → Editor mode. To set the mode directly in your config file, set the [`editorMode`](./code-settings.md#global-config-settings) global config key to `"vim"` in `~/.claude.json`.
...
See [Interactive mode](./code-interactive-mode.md#vim-editor-mode) for the complete reference.
