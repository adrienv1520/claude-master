...
**Plan in the cloud with ultraplan**: To draft and review the plan itself in a web session, use [ultraplan](./code-ultraplan.md). Claude generates the plan on Claude Code on the web while you keep working, then you comment on sections in your browser and choose to execute remotely or send the plan back to your terminal.
...
display name is visible to all recipients with access. [Claude in Slack](./code-slack.md)
...
Run Claude on a recurring schedule to automate work like daily PR reviews, dependency audits, and CI failure analysis. See [Schedule tasks on the web](./code-web-scheduled-tasks.md) for the full guide.
...
Use a setup script to install things the cloud needs but your laptop already has, like a language runtime or CLI tool. Use a [SessionStart hook](./code-hooks.md#sessionstart) for project setup that should run everywhere, cloud and local, like `npm install`.
...
Custom environment images and snapshots are not yet supported. Use [setup scripts](#setup-scripts) to install packages when a session starts, or [SessionStart hooks](./code-hooks.md#sessionstart) for dependency installation that should also run in local environments. SessionStart hooks have [known limitations](#dependency-management-limitations).
...
SessionStart hooks can persist environment variables for subsequent Bash commands by writing to the file specified in the `CLAUDE_ENV_FILE` environment variable. For details, see [SessionStart hooks](./code-hooks.md#sessionstart) in the hooks reference.
...
* **Platform restrictions**: Claude Code on the web only works with code hosted in GitHub. Self-hosted [GitHub Enterprise Server](./code-github-enterprise-server.md) instances are supported for Team and Enterprise plans. GitLab and other non-GitHub repositories cannot be used with cloud sessions

## Best practices

1. **Automate environment setup**: Use [setup scripts](#setup-scripts) to install dependencies and configure tools before Claude Code launches. For more advanced scenarios, configure [SessionStart hooks](./code-hooks.md#sessionstart).
...
* [Hooks configuration](./code-hooks.md)
* [Settings reference](./code-settings.md)
