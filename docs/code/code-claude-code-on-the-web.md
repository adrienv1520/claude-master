...
Custom environment images and snapshots are not yet supported. As a workaround, you can use [SessionStart hooks](./code-hooks.md#sessionstart) to install packages when a session starts. This approach has [known limitations](#dependency-management-limitations).
...
1. **Use Claude Code hooks**: Configure [SessionStart hooks](./code-hooks.md#sessionstart) to automate environment setup and dependency installation.
