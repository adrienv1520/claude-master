...
Click the dropdown at the top of the Claude Code panel to access your conversation history. You can search by keyword or browse by time (Today, Yesterday, Last 7 days, etc.). Click any conversation to resume it with the full message history. For more on resuming sessions, see [Common workflows](./code-common-workflows.md#resume-previous-conversations).
...
| ------------------- | --------------------------------------------- | ---------------------------------------- |
| Commands and skills | [All](./code-interactive-mode.md#built-in-commands) | Subset (type `/` to see available)       |
...
Once configured, ask Claude to use the tools (e.g., "Review PR #456"). Some servers require authentication: run `claude` in the terminal, then type `/mcp` to authenticate. See the [MCP documentation](./code-mcp.md) for available servers.
...
For detailed git workflows including PR reviews and branch management, see [Common workflows](./code-common-workflows.md#create-pull-requests).
...
* [Set up MCP servers](./code-mcp.md) to extend Claude's capabilities with external tools. Configure servers using the CLI, then use them in the extension.
