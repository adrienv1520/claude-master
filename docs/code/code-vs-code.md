...
For more ideas on what you can do with Claude Code, see [Common workflows](./code-common-workflows.md).
...
* **Command menu**: click `/` or type `/` to open the command menu. Options include attaching files, switching models, toggling extended thinking, viewing plan usage (`/usage`), and starting a [Remote Control](./code-remote-control.md) session (`/remote-control`). The Customize section provides access to MCP servers, hooks, memory, permissions, and plugins. Items with a terminal icon open in the integrated terminal.
...
* **Extended thinking**: lets Claude spend more time reasoning through complex problems. Toggle it on via the command menu (`/`). See [Extended thinking](./code-common-workflows.md#use-extended-thinking-thinking-mode) for details.
...
Click the dropdown at the top of the Claude Code panel to access your conversation history. You can search by keyword or browse by time (Today, Yesterday, Last 7 days, etc.). Click any conversation to resume it with the full message history. New sessions receive AI-generated titles based on your first message. Hover over a session to reveal rename and remove actions: rename to give it a descriptive title, or remove to delete it from the list. For more on resuming sessions, see [Common workflows](./code-common-workflows.md#resume-previous-conversations).

### Resume remote sessions from Claude.ai

If you use [Claude Code on the web](./code-claude-code-on-the-web.md), you can resume those remote sessions directly in VS Code. This requires signing in with **Claude.ai Subscription**, not Anthropic Console.
...
The VS Code extension includes a graphical interface for installing and managing [plugins](./code-plugins.md). Type `/plugins` in the prompt box to open the **Manage plugins** interface.
...
For more about the plugin system, see [Plugins](./code-plugins.md) and [Plugin marketplaces](./code-plugin-marketplaces.md).
...
For setup instructions, the full list of capabilities, and troubleshooting, see [Use Claude Code with Chrome](./code-chrome.md).
...
* **Claude Code settings** in `~/.claude/settings.json`: shared between the extension and CLI. Use for allowed commands, environment variables, hooks, and MCP servers. See [Settings](./code-settings.md) for details.
...
| `initialPermissionMode`           | `default` | Controls approval prompts for new conversations: `default`, `plan`, `acceptEdits`, or `bypassPermissions`. See [permission modes](./code-permission-modes.md).                                                                                                                                                                                                            |
...
| ------------------- | ------------------- | ------------------------------------------------------------------------------------ |
| Commands and skills | [All](./code-commands.md) | Subset (type `/` to see available)                                                   |
...
For full details on how checkpoints work and their limitations, see [Checkpointing](./code-checkpointing.md).
...
To manage MCP servers without leaving VS Code, type `/mcp` in the chat panel. The MCP management dialog lets you enable or disable servers, reconnect to a server, and manage OAuth authentication. See the [MCP documentation](./code-mcp.md) for available servers.
...
<Steps>
  ...
  <Step title="Configure your provider">
    ...
    * [Claude Code on Amazon Bedrock](./code-amazon-bedrock.md)
    * [Claude Code on Google Vertex AI](./code-google-vertex-ai.md)
    * [Claude Code on Microsoft Foundry](./code-microsoft-foundry.md)
...
Your code stays private. Claude Code processes your code to provide assistance but does not use it to train models. For details on data handling and how to opt out of logging, see [Data and privacy](./code-data-usage.md).
...
For additional help, see the [troubleshooting guide](./code-troubleshooting.md).

## Next steps

Now that you have Claude Code set up in VS Code:

* [Explore common workflows](./code-common-workflows.md) to get the most out of Claude Code
* [Set up MCP servers](./code-mcp.md) to extend Claude's capabilities with external tools. Add servers using the CLI, then manage them with `/mcp` in the chat panel.
* [Configure Claude Code settings](./code-settings.md) to customize allowed commands, hooks, and more. These settings are shared between the extension and CLI.
