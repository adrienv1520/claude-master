...
* **Model**: pick a [model](./code-overview.md#models) from the dropdown next to the send button. The model is locked once the session starts.
...
The `delegate` and `dontAsk` permission modes are available only in the [CLI](./code-permissions.md#permission-modes).

<Tip title="Best practice">
  Start complex tasks in Plan mode so Claude maps out an approach before making changes. Once you approve the plan, switch to Code or Ask mode to execute it. See [explore first, then plan, then code](./code-best-practices.md#explore-first-then-plan-then-code) for more on this workflow.
</Tip>
...
Use the filter icon at the top of the sidebar to filter sessions by status (Active, Archived) and environment (Local, Cloud). To rename a session or check context usage, click the session title in the toolbar at the top of the active session. When context fills up, Claude automatically summarizes the conversation and continues working. You can also type "compact this conversation" to trigger summarization earlier and free up context space. See [the context window](./code-how-claude-code-works.md#the-context-window) for details on how compaction works.
...
[Extended thinking](./code-common-workflows.md#use-extended-thinking-thinking-mode) is enabled by default, which improves performance on complex reasoning tasks but uses additional tokens. To disable it or adjust the budget, set `MAX_THINKING_TOKENS` in your shell profile. Use `0` to disable.
...
| Key                                        | Description                                                                                                      |
| ------------------------------------------ | ---------------------------------------------------------------------------------------------------------------- |
| `permissions.disableBypassPermissionsMode` | set to `"disable"` to prevent users from enabling Act mode. See [permissions](./code-permissions.md#managed-settings). |
...
* **Agent teams and `delegate` mode**: multi-agent orchestration and the `delegate` permission mode are available via the [CLI](./code-agent-teams.md) and [Agent SDK](./code-headless.md), not in Desktop.
