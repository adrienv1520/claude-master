...
You can use Claude Code through the terminal ([CLI](./code-quickstart.md)) or through the desktop app described here. Both provide the same core capabilities. The desktop app adds a graphical interface and visual session management.
...
<Steps>
  <Step title="Choose a folder and environment">
    ...
    You can also run [remote sessions](./code-claude-code-on-the-web.md) that continue in the cloud even if you close the app.
  </Step>
...
Remote sessions only support **Code** and **Plan** modes because they continue running in the background without requiring your active participation. See [permission modes](./code-permissions.md#permission-modes) for details on how these work internally.
...
Remote sessions support **Code** and **Plan** modes. See [Claude Code on the web](./code-claude-code-on-the-web.md) for details on configuring remote environments.
...
Connectors are [MCP (Model Context Protocol) servers](./code-mcp.md) with built-in setup. You can also [create custom connectors](https://support.claude.com/en/articles/11175166-getting-started-with-custom-connectors-using-remote-mcp) or add MCP servers manually via [configuration files](./code-mcp.md#configure-mcp-servers).

### Create custom skills

[Skills](./code-skills.md) are reusable prompts that extend Claude's capabilities. For example, you could create a `review` skill that runs your standard code review checklist, or a `deploy` skill that walks through your deployment steps. Skills are defined as markdown files in `.claude/skills/` and can include instructions, context, and even call other tools. Ask Claude what skills are available or to run a specific skill. Claude can also help you create a skill if you describe what you want, or see [skills](./code-skills.md) to learn how to write them yourself.

### Automate workflows with hooks

[Hooks](./code-hooks.md) run shell commands automatically in response to Claude Code events. For example, you could run a linter after every file edit, auto-format code, or send notifications when tasks complete. Hooks are configured in your [settings files](./code-settings.md). See [hooks](./code-hooks.md) for available events and configuration examples.

## Environment configuration

When starting a session, you choose between **Local** (runs on your machine) or **Remote** (runs on Anthropic's cloud).

**Local sessions** inherit environment variables from your shell. If you need additional variables, set them in your shell profile (`~/.zshrc`, `~/.bashrc`) and restart the desktop app. See [environment variables](./code-settings.md#environment-variables) for the full list of supported variables.
...
**Remote sessions** run on Anthropic's cloud infrastructure and continue even if you close the app. Usage counts toward your subscription plan limits with no separate compute charges. See [Claude Code on the web](./code-claude-code-on-the-web.md) for details on configuring remote environments.
...
* **[Settings](./code-settings.md)** in `~/.claude.json` and `~/.claude/settings.json` are shared
* **Models** (Sonnet, Opus, Haiku) are available in both (Desktop requires selecting before starting a session)

<Note>
  MCP servers configured for the **Claude Desktop chat app** (in `claude_desktop_config.json`) are separate from Claude Code. To use MCP servers in Claude Code, configure them in `~/.claude.json` or your project's `.mcp.json` file. See [MCP configuration](./code-mcp.md#configure-mcp-servers) for details.
</Note>
...
* [Third-party API providers](./code-third-party-integrations.md) (Bedrock, Vertex, Foundry). If you use these, continue using CLI for those projects.
* [CLI flags](./code-cli-reference.md) for scripting (`--print`, `--resume`, `--continue`)
* [Programmatic usage](./code-headless.md) via the Agent SDK

## Troubleshooting

Solutions to common issues with the Claude desktop app. For CLI issues, see [CLI troubleshooting](./code-troubleshooting.md).
...
* [Claude Code on the web](./code-claude-code-on-the-web.md): Run remote sessions that continue in the cloud
* [CLI reference](./code-cli-reference.md): Use Claude Code in your terminal with flags and scripting
* [Common workflows](./code-common-workflows.md): Tutorials for debugging, refactoring, testing, and more
* [Settings reference](./code-settings.md): Configure Claude Code behavior with settings files
