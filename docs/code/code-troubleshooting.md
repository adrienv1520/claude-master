...
<Tip>
  If you'd rather skip the terminal entirely, the [Claude Code Desktop app](./code-desktop-quickstart.md) lets you install and use Claude Code through a graphical interface. Download it for [macOS](https://claude.ai/api/desktop/darwin/universal/dmg/latest/redirect?utm_source=claude_code\&utm_medium=docs) or [Windows](https://claude.com/download?utm_source=claude_code\&utm_medium=docs) and start coding without any command-line setup.
</Tip>
...
On Linux, check for missing shared libraries. If `ldd` shows missing libraries, you may need to install system packages. On Alpine Linux and other musl-based distributions, see [Alpine Linux setup](./code-setup.md#alpine-linux-and-musl-based-distributions).
...
2. **If behind a proxy**, set `HTTPS_PROXY` so the installer can route through it. See [proxy configuration](./code-network-config.md#proxy-configuration) for details.
   ```bash  theme={null}
   export HTTPS_PROXY=http://proxy.example.com:8080
   curl -fsSL https://claude.ai/install.sh | bash
   ```
...
**If Git is already installed** but Claude Code still can't find it, set the path in your [settings.json file](./code-settings.md):
...
[Sandboxing](./code-sandboxing.md) is supported on WSL2 but requires installing additional packages. If you see an error about missing `bubblewrap` or `socat` when running `/sandbox`, install the dependencies:
...
Sandboxed commands cannot launch Windows binaries such as `cmd.exe`, `powershell.exe`, or executables under `/mnt/c/`. WSL hands these off to the Windows host over a Unix socket, which the sandbox blocks. If a command needs to invoke a Windows binary, add it to [`excludedCommands`](./code-settings.md#sandbox-settings) so it runs outside the sandbox.
...
to run without approval using the `/permissions` command. See [Permissions docs](./code-permissions.md#manage-permissions).
...
* **Behind a proxy**: corporate proxies can interfere with API requests. See [network configuration](./code-network-config.md) for proxy setup.
...
| `managed-mcp.json`            | [Managed MCP servers](./code-mcp.md#managed-mcp-configuration)                                               |
| Managed settings              | [Managed settings](./code-settings.md#settings-files) (server-managed, MDM/OS-level policies, or file-based) |
...
Then set `USE_BUILTIN_RIPGREP=0` in your [environment](./code-env-vars.md).
...
For additional JetBrains configuration tips, see the [JetBrains IDE guide](./code-jetbrains.md#plugin-settings).
...
2. **Use post-processing hooks**: set up automatic formatting hooks to detect and add missing language tags. See [Auto-format code after edits](./code-hooks-guide.md#auto-format-code-after-edits) for an example of a PostToolUse formatting hook.
...
3. **Specify formatting preferences**: include formatting requirements in your prompts or project [memory](./code-memory.md) files.
...
* **Use project conventions**: document your preferred markdown style in [`CLAUDE.md`](./code-memory.md)
