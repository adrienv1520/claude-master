...
For detailed permission configuration, see [Identity and Access Management](./code-iam.md).
...
* **Command blocklist**: Blocks risky commands that fetch arbitrary content from the web like `curl` and `wget` by default. When explicitly allowed, be aware of [permission pattern limitations](./code-iam.md#tool-specific-permission-rules)
...
* **Secure credential storage**: API keys and tokens are encrypted. See [Credential Management](./code-iam.md#credential-management)
...
* Use [managed settings](./code-iam.md#managed-settings) to enforce organizational standards
...
* [Identity and Access Management](./code-iam.md) - Configure permissions and access controls
