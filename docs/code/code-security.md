...
For detailed permission configuration, see [Permissions](./code-permissions.md).

### Built-in protections

To mitigate risks in agentic systems:

* **Sandboxed bash tool**: [Sandbox](./code-sandboxing.md) bash commands with filesystem and network isolation, reducing permission prompts while maintaining security. Enable with `/sandbox` to define boundaries where Claude Code can work autonomously
...
* **Command blocklist**: Blocks risky commands that fetch arbitrary content from the web like `curl` and `wget` by default. When explicitly allowed, be aware of [permission pattern limitations](./code-permissions.md#tool-specific-permission-rules)
...
* **Secure credential storage**: API keys and tokens are encrypted. See [Credential Management](./code-authentication.md#credential-management)
...
See [VS Code security and privacy](./code-vs-code.md#security-and-privacy) for more information on running Claude Code in an IDE.

## Cloud execution security

When using [Claude Code on the web](./code-claude-code-on-the-web.md), additional security controls are in place:
...
* Consider using [devcontainers](./code-devcontainer.md) for additional isolation
...
* Audit or block settings changes during sessions with [`ConfigChange` hooks](./code-hooks.md#configchange)
...
* [Permissions](./code-permissions.md) - Configure permissions and access controls
* [Monitoring usage](./code-monitoring-usage.md) - Track and audit Claude Code activity
* [Development containers](./code-devcontainer.md) - Secure, isolated environments
