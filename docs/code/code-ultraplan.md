...
Ultraplan hands a planning task from your local CLI to a [Claude Code on the web](./code-claude-code-on-the-web.md) session running in [plan mode](./code-permission-modes.md#analyze-before-you-edit-with-plan-mode). Claude drafts the plan in the cloud while you keep working in your terminal. When the plan is ready, you open it in your browser to comment on specific sections, ask for revisions, and choose where to execute it.
...
Ultraplan requires a [Claude Code on the web](./code-claude-code-on-the-web.md#who-can-use-claude-code-on-the-web) account and a GitHub repository. The cloud session runs in your account's default [cloud environment](./code-claude-code-on-the-web.md#cloud-environment).
...
The command and keyword paths open a confirmation dialog before launching. The local plan path skips this dialog because that selection already serves as confirmation. If [Remote Control](./code-remote-control.md) is active, it disconnects when ultraplan starts because both features occupy the claude.ai/code interface and only one can be connected at a time.
...
Select **Approve Claude's plan and start coding** in your browser to have Claude implement it in the same Claude Code on the web session. Your terminal shows a confirmation, the status indicator clears, and the work continues in the cloud. When the implementation finishes, [review the diff](./code-claude-code-on-the-web.md#review-changes-with-diff-view) and create a pull request from the web interface.
...
* [Claude Code on the web](./code-claude-code-on-the-web.md): the cloud infrastructure ultraplan runs on
* [Plan mode](./code-permission-modes.md#analyze-before-you-edit-with-plan-mode): how planning works in a local session
* [Remote Control](./code-remote-control.md): use the claude.ai/code interface with a session running on your own machine
