...
* **Command blocklist**: Blocks risky commands that fetch arbitrary content from the web like `curl` and `wget` by default. When explicitly allowed, be aware of [permission pattern limitations](./iam.md#tool-specific-permission-rules)
...
See [here](./vs-code.md#security) for more information on the security of running Claude Code in an IDE.
...
* Consider using [devcontainers](./devcontainer.md) for additional isolation
