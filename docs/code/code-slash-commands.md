...
| `/cost`                   | Show token usage statistics. See [cost tracking guide](./code-costs.md#using-the-cost-command) for subscription-specific details. |
...
| `/permissions`            | View or update [permissions](./code-iam.md#configuring-permissions)                                                               |
...
Include file contents in commands using the `@` prefix to [reference files](./code-common-workflows.md#reference-files-and-directories).
...
Slash commands can trigger extended thinking by including [extended thinking keywords](./code-common-workflows.md#use-extended-thinking).
...
| `agent`                    | Specify which [agent type](./code-sub-agents.md#built-in-subagents) to use when `context: fork` is set. Only applicable when combined with `context: fork`.                                 | `general-purpose`                   |
...
[Plugins](./code-plugins.md) can provide custom slash commands that integrate seamlessly with Claude Code. Plugin commands work exactly like user-defined commands but are distributed through [plugin marketplaces](./code-plugin-marketplaces.md).
...
See [MCP permission rules](./code-iam.md#tool-specific-permission-rules) for more details.
...
The `Skill` tool allows Claude to programmatically invoke both [custom slash commands](./code-slash-commands.md#custom-slash-commands) and [Agent Skills](./code-skills.md) during a conversation. This gives Claude the ability to use these capabilities on your behalf when appropriate.
...
<Note>
  The `user-invocable` field in Skills only controls menu visibility, not `Skill` tool access. Use `disable-model-invocation: true` to block programmatic invocation. See [Control Skill visibility](./code-skills.md#control-skill-visibility) for details.
</Note>
...
Learn more about [Agent Skills](./code-skills.md).

## See also

* [Plugins](./code-plugins.md) - Extend Claude Code with custom commands through plugins
* [Identity and Access Management](./code-iam.md) - Complete guide to permissions, including MCP tool permissions
...
* [Memory management](./code-memory.md) - Managing Claude's memory across sessions
