...
* **User confirmation**: New domain requests trigger permission prompts (unless [`allowManagedDomainsOnly`](./code-settings.md#sandbox-settings) is enabled, which blocks non-allowed domains automatically)
...
By default, if the sandbox cannot start (missing dependencies, unsupported platform, or platform restrictions), Claude Code shows a warning and runs commands without sandboxing. To make this a hard failure instead, set [`sandbox.failIfUnavailable`](./code-settings.md#sandbox-settings) to `true`. This is intended for managed deployments that require sandboxing as a security gate.
...
Customize sandbox behavior through your `settings.json` file. See [Settings](./code-settings.md#sandbox-settings) for complete configuration reference.
...
<Note>
  ...
  You can disable this escape hatch by setting `"allowUnsandboxedCommands": false` in your [sandbox settings](./code-settings.md#sandbox-settings). When disabled, the `dangerouslyDisableSandbox` parameter is completely ignored and all commands must run sandboxed or be explicitly listed in `excludedCommands`.
</Note>
...
* Cannot read files that are denied in your [Claude permission settings](./code-permissions.md#manage-permissions)
...
Sandboxing and [permissions](./code-permissions.md) are complementary security layers that work together:
...
* **Built-in file tools**: Read, Edit, and Write use the permission system directly rather than running through the sandbox. See [permissions](./code-permissions.md).
* **Computer use**: when Claude opens apps and controls your screen, it runs on your actual desktop rather than in an isolated environment. Per-app permission prompts gate each application. See [computer use in the CLI](./code-computer-use.md) or [computer use in Desktop](./code-desktop.md#let-claude-use-your-computer).

## See also

* [Security](./code-security.md) - Comprehensive security features and best practices
* [Permissions](./code-permissions.md) - Permission configuration and access control
