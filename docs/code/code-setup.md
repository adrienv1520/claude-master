...
This page covers system requirements, platform-specific installation details, updates, and uninstallation. For a guided walkthrough of your first session, see the [quickstart](./code-quickstart.md). If you've never used a terminal before, see the [terminal guide](./code-terminal-guide.md).
...
* **Operating system**:
  ...
  * Alpine Linux 3.19+
* **Hardware**: 4 GB+ RAM
* **Network**: internet connection required. See [network configuration](./code-network-config.md#network-access-requirements).
...
* **ripgrep**: usually included with Claude Code. If search fails, see [search troubleshooting](./code-troubleshooting.md#search-and-discovery-issues).

## Install Claude Code

<Tip>
  Prefer a graphical interface? The [Desktop app](./code-desktop-quickstart.md) lets you use Claude Code without the terminal. Download it for [macOS](https://claude.ai/api/desktop/darwin/universal/dmg/latest/redirect?utm_source=claude_code\&utm_medium=docs) or [Windows](https://claude.ai/api/desktop/win32/x64/exe/latest/redirect?utm_source=claude_code\&utm_medium=docs).

  New to the terminal? See the [terminal guide](./code-terminal-guide.md) for step-by-step instructions.
</Tip>
...
If you encounter any issues during installation, see the [troubleshooting guide](./code-troubleshooting.md).
...
If Claude Code can't find your Git Bash installation, set the path in your [settings.json file](./code-settings.md):
...
Claude Code can also run PowerShell natively on Windows as an opt-in preview. See [PowerShell tool](./code-tools-reference.md#powershell-tool) for setup and limitations.

**Option 2: WSL**

Both WSL 1 and WSL 2 are supported. WSL 2 supports [sandboxing](./code-sandboxing.md) for enhanced security. WSL 1 does not support sandboxing.
...
Then set `USE_BUILTIN_RIPGREP` to `0` in your [`settings.json`](./code-settings.md#available-settings) file:
...
For a more detailed check of your installation and configuration, run [`claude doctor`](./code-troubleshooting.md#get-more-help):
...
Claude Code requires a Pro, Max, Teams, Enterprise, or Console account. The free Claude.ai plan does not include Claude Code access. You can also use Claude Code with a third-party API provider like [Amazon Bedrock](./code-amazon-bedrock.md), [Google Vertex AI](./code-google-vertex-ai.md), or [Microsoft Foundry](./code-microsoft-foundry.md).

After installing, log in by running `claude` and following the browser prompts. See [Authentication](./code-authentication.md) for all account types and team setup options.
...
Configure this via `/config` → **Auto-update channel**, or add it to your [settings.json file](./code-settings.md):
...
For enterprise deployments, you can enforce a consistent release channel across your organization using [managed settings](./code-permissions.md#managed-settings).

### Disable auto-updates

Set `DISABLE_AUTOUPDATER` to `"1"` in the `env` key of your [`settings.json`](./code-settings.md#available-settings) file:
...
<Warning>
  Do NOT use `sudo npm install -g` as this can lead to permission issues and security risks. If you encounter permission errors, see [troubleshooting permission errors](./code-troubleshooting.md#permission-errors-during-installation).
</Warning>
