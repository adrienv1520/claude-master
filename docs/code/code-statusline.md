...
Add a `statusLine` field to your user settings (`~/.claude/settings.json`, where `~` is your home directory) or [project settings](./code-settings.md#settings-files). Set `type` to `"command"` and point `command` to a script path or an inline shell command. For a full walkthrough of creating a script, see [Build a status line step by step](#build-a-status-line-step-by-step).
...
| `vim.mode`                                                                | Current vim mode (`NORMAL` or `INSERT`) when [vim mode](./code-interactive-mode.md#vim-editor-mode) is enabled                                                                                     |
