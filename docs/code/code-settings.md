...
* **Managed settings**: For organizations that need centralized control, Claude Code supports `managed-settings.json` and `managed-mcp.json` files that can be deployed to system directories:
  ...
  * Windows: `C:\Program Files\ClaudeCode\`
  ...
  See [Managed settings](./code-iam.md#managed-settings) and [Managed MCP configuration](./code-mcp.md#managed-mcp-configuration) for details.
...
| `deny`                         | Array of permission rules to deny tool use. Use this to exclude sensitive files from Claude Code access. See [Permission rule syntax](#permission-rule-syntax) and [Bash permission limitations](./code-iam.md#tool-specific-permission-rules) | `[ "WebFetch", "Bash(curl *)", "Read(./.env)", "Read(./secrets/**)" ]` |
| `additionalDirectories`        | Additional [working directories](./code-iam.md#working-directories) that Claude has access to                                                                                                                                                  | `[ "../docs/" ]`                                                       |
| `defaultMode`                  | Default [permission mode](./code-iam.md#permission-modes) when opening Claude Code                                                                                                                                                             | `"acceptEdits"`                                                        |
| `disableBypassPermissionsMode` | Set to `"disable"` to prevent `bypassPermissions` mode from being activated. This disables the `--dangerously-skip-permissions` command-line flag. See [managed settings](./code-iam.md#managed-settings)                                      | `"disable"`                                                            |
...
<Warning>
  Bash permission patterns that try to constrain command arguments are fragile. For example, `Bash(curl http://github.com/ *)` intends to restrict curl to GitHub URLs, but won't match `curl -X GET http://github.com/...` (flags before URL), `curl https://github.com/...` (different protocol), or commands using shell variables. Do not rely on argument-constraining patterns as a security boundary. See [Bash permission limitations](./code-iam.md#tool-specific-permission-rules) for alternatives.
</Warning>

For detailed information about tool-specific permission patterns—including Read, Edit, WebFetch, MCP, Task rules, and Bash permission limitations—see [Tool-specific permission rules](./code-iam.md#tool-specific-permission-rules).
...
**Managed settings only**: Controls which plugin marketplaces users are allowed to add. This setting can only be configured in [`managed-settings.json`](./code-iam.md#managed-settings) and provides administrators with strict control over marketplace sources.
...
Permission rules can be configured using `/allowed-tools` or in [permission settings](./code-settings.md#available-settings). Also see [Tool-specific permission rules](./code-iam.md#tool-specific-permission-rules).
...
* [Identity and Access Management](./code-iam.md#configuring-permissions) - Permission system overview and how allow/ask/deny rules interact
* [Tool-specific permission rules](./code-iam.md#tool-specific-permission-rules) - Detailed patterns for Bash, Read, Edit, WebFetch, MCP, and Task tools, including security limitations
* [Managed settings](./code-iam.md#managed-settings) - Managed policy configuration for organizations
