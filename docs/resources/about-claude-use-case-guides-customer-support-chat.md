...
Work with your support team to [define clear success criteria](../developer/test-and-evaluate-define-success.md) and write [detailed evaluations](../developer/test-and-evaluate-develop-tests.md) with measurable benchmarks and goals.
...
It's hard to know how well your prompt works without deploying it in a test production setting and [running evaluations](../developer/test-and-evaluate-develop-tests.md) so let's build a small application using our prompt, the Anthropic SDK, and streamlit for a user interface.
...
Prompting often requires testing and optimization for it to be production ready. To determine the readiness of your solution, evaluate the chatbot performance using a systematic process combining quantitative and qualitative methods. Creating a [strong empirical evaluation](../developer/test-and-evaluate-develop-tests.md#building-evals-and-test-cases) based on your defined success criteria will allow you to optimize your prompts. 
...
- [Reduce hallucination](../developer/test-and-evaluate-strengthen-guardrails-reduce-hallucinations.md): Implement fact-checking mechanisms and [citations](https://github.com/anthropics/anthropic-cookbook/blob/main/misc/using_citations.ipynb) to ground responses in provided information.
...
- [Mitigate jailbreaks](../developer/test-and-evaluate-strengthen-guardrails-mitigate-jailbreaks.md): Use methods like harmlessness screens and input validation to prevent users from exploiting model vulnerabilities, aiming to generate inappropriate content.
...
- [Keep Claude in character](../developer/test-and-evaluate-strengthen-guardrails-keep-claude-in-character.md): Prevent Claude from changing their style of context, even during long, complex interactions.
...
1. Use the [Anthropic Streaming API](../developer/build-with-claude-streaming.md) to support streaming responses.
...
- Ensure that you are making the most out of your prompts and optimizing through prompt engineering. Use our [prompt engineering guides](../developer/build-with-claude-prompt-engineering-overview.md) to write the most effective prompts.
- Add additional [tools](https://platform.claude.com/docs/en/build-with-claude/tool-use.md) to the prompt (which can include [prompt chains](../developer/build-with-claude-prompt-engineering-chain-prompts.md)) and see if you can achieve the functionality required.
