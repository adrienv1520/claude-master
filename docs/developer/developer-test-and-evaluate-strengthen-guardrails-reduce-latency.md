...
For a more in-depth understanding of these terms, check out our [glossary](../resources/resources-about-claude-glossary.md).
...
One of the most straightforward ways to reduce latency is to select the appropriate model for your use case. Anthropic offers a [range of models](./developer-about-claude-models-overview.md) with different capabilities and performance characteristics. Consider your specific requirements and choose the model that best fits your needs in terms of speed and output quality.
...
For more details about model metrics, see our [models overview](./developer-about-claude-models-overview.md) page.
...
- **Be clear but concise**: Aim to convey your intent clearly and concisely in the prompt. Avoid unnecessary details or redundant information, while keeping in mind that [claude lacks context](./developer-build-with-claude-prompt-engineering-be-clear-and-direct.md) on your use case and may not make the intended leaps of logic if instructions are unclear.
- **Ask for shorter responses:**: Ask Claude directly to be concise. The Claude 3 family of models has improved steerability over previous generations. If Claude is outputting unwanted length, ask Claude to [curb its chattiness](./developer-build-with-claude-prompt-engineering-be-clear-and-direct.md).
  <Tip> Due to how LLMs count [tokens](../resources/resources-about-claude-glossary.md#tokens) instead of words, asking for an exact word count or a word count limit is not as effective a strategy as asking for paragraph or sentence count limits.</Tip>
...
- **Experiment with temperature**: The `temperature` [parameter](../api/api-messages.md) controls the randomness of the output. Lower values (e.g., 0.2) can sometimes lead to more focused and shorter responses, while higher values (e.g., 0.8) may result in more diverse but potentially longer outputs.
...
Visit [streaming Messages](./developer-build-with-claude-streaming.md) to learn about how you can implement streaming for your use case.