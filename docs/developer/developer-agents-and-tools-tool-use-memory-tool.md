...
The memory tool can be combined with [context editing](./developer-build-with-claude-context-editing.md), which automatically clears old tool results when conversation context grows beyond a configured threshold. This combination enables long-running agentic workflows that would otherwise exceed context limits.
