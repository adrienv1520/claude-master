...
3. **Execute tools.** The SDK runs each requested tool and collects the results. Each set of tool results feeds back to Claude for the next decision. You can use [hooks](./developer-agent-sdk-hooks.md) to intercept, modify, or block tool calls before they run.
...
- **`SystemMessage`:** session lifecycle events. The `subtype` field distinguishes them: `"init"` is the first message (session metadata), and `"compact_boundary"` fires after [compaction](#automatic-compaction). In TypeScript, the compact boundary is its own [`SDKCompactBoundaryMessage`](./developer-agent-sdk-typescript.md#sdk-compact-boundary-message) type rather than a subtype of `SDKSystemMessage`.
...
- **`StreamEvent`:** only emitted when partial messages are enabled. Contains raw API streaming events (text deltas, tool input chunks). See [Stream responses](./developer-agent-sdk-streaming-output.md).
...
These five types cover the full agent loop lifecycle in both SDKs. The TypeScript SDK also yields additional observability events (hook events, tool progress, rate limits, task notifications) that provide extra detail but are not required to drive the loop. See the [Python message types reference](./developer-agent-sdk-python.md#message-types) and [TypeScript message types reference](./developer-agent-sdk-typescript.md#message-types) for the complete lists.
...
- **Live streaming:** enable partial messages (`include_partial_messages` in Python, `includePartialMessages` in TypeScript) to get `StreamEvent` messages in real time. See [Stream responses in real-time](./developer-agent-sdk-streaming-output.md).
...
- **Connect external services** with [MCP servers](./developer-agent-sdk-mcp.md) (databases, browsers, APIs)
- **Define custom tools** with [custom tool handlers](./developer-agent-sdk-custom-tools.md)
- **Load project skills** via [setting sources](./developer-agent-sdk-claude-code-features.md) for reusable workflows
...
- **`disallowed_tools` / `disallowedTools`** blocks listed tools, regardless of other settings. See [Permissions](./developer-agent-sdk-permissions.md) for the order that rules are checked before a tool runs.
...
You can also scope individual tools with rules like `"Bash(npm:*)"` to allow only specific commands. See [Permissions](./developer-agent-sdk-permissions.md) for the full rule syntax.
...
Custom tools default to sequential execution. To enable parallel execution for a custom tool, mark it as read-only in its annotations: `readOnly` in [TypeScript](./developer-agent-sdk-typescript.md#tool) or `readOnlyHint` in [Python](./developer-agent-sdk-python.md#tool).

## Control how the loop runs

You can limit how many turns the loop takes, how much it costs, how deeply Claude reasons, and whether tools require approval before running. All of these are fields on [`ClaudeAgentOptions`](./developer-agent-sdk-python.md#claude-agent-options) (Python) / [`Options`](./developer-agent-sdk-typescript.md#options) (TypeScript).
...
When either limit is hit, the SDK returns a `ResultMessage` with a corresponding error subtype (`error_max_turns` or `error_max_budget_usd`). See [Handle the result](#handle-the-result) for how to check these subtypes and [`ClaudeAgentOptions`](./developer-agent-sdk-python.md#claude-agent-options) / [`Options`](./developer-agent-sdk-typescript.md#options) for syntax.

### Effort level

The `effort` option controls how much reasoning Claude applies. Lower effort levels use fewer tokens per turn and reduce cost. Not all models support the effort parameter. See [Effort](./developer-build-with-claude-effort.md) for which models support it.
...
For interactive applications, use `"default"` with a tool approval callback to surface approval prompts. For autonomous agents on a dev machine, `"acceptEdits"` auto-approves file edits while still gating `Bash` behind allow rules. Reserve `"bypassPermissions"` for CI, containers, or other isolated environments. See [Permissions](./developer-agent-sdk-permissions.md) for full details.

### Model

If you don't set `model`, the SDK uses Claude Code's default, which depends on your authentication method and subscription. Set it explicitly (for example, `model="claude-sonnet-4-6"`) to pin a specific model or to use a smaller model for faster, cheaper agents. See [models](https://platform.claude.com/docs/en/about-claude/models.md) for available IDs.

## The context window

The context window is the total amount of information available to Claude during a session. It does not reset between turns within a session. Everything accumulates: the system prompt, tool definitions, conversation history, tool inputs, and tool outputs. Content that stays the same across turns (system prompt, tool definitions, CLAUDE.md) is automatically [prompt cached](./developer-build-with-claude-prompt-caching.md), which reduces cost and latency for repeated prefixes.
...
| **CLAUDE.md files** | Session start, when [`settingSources`](./developer-agent-sdk-claude-code-features.md) is enabled | Full content in every request (but prompt-cached, so only the first request pays full cost) |
| **Tool definitions** | Every request | Each tool adds its schema; use [MCP tool search](./developer-agent-sdk-mcp.md#mcp-tool-search) to load tools on-demand instead of all at once |
...
Compaction replaces older messages with a summary, so specific instructions from early in the conversation may not be preserved. Persistent rules belong in CLAUDE.md (loaded via [`settingSources`](./developer-agent-sdk-claude-code-features.md)) rather than in the initial prompt, because CLAUDE.md content is re-injected on every request.
...
- **`PreCompact` hook:** Run custom logic before compaction occurs, for example to archive the full transcript. The hook receives a `trigger` field (`manual` or `auto`). See [hooks](./developer-agent-sdk-hooks.md).
- **Manual compaction:** Send `/compact` as a prompt string to trigger compaction on demand. (Slash commands sent this way are SDK inputs, not CLI-only shortcuts. See [slash commands in the SDK](./developer-agent-sdk-slash-commands.md).)
...
See [Session management](./developer-agent-sdk-sessions.md) for the full guide on resume, continue, and fork patterns.

<Note>
In Python, `ClaudeSDKClient` handles session IDs automatically across multiple calls. See the [Python SDK reference](./developer-agent-sdk-python.md#choosing-between-query-and-claude-sdk-client) for details.
</Note>
...
The `result` field (the final text output) is only present on the `success` variant, so always check the subtype before reading it. All result subtypes carry `total_cost_usd`, `usage`, `num_turns`, and `session_id` so you can track cost and resume even after errors. In Python, `total_cost_usd` and `usage` are typed as optional and may be `None` on some error paths, so guard before formatting them. See [Tracking costs and usage](./developer-agent-sdk-cost-tracking.md) for details on interpreting the `usage` fields.

The result also includes a `stop_reason` field (`string | null` in TypeScript, `str | None` in Python) indicating why the model stopped generating on its final turn. Common values are `end_turn` (model finished normally), `max_tokens` (hit the output token limit), and `refusal` (the model declined the request). On error result subtypes, `stop_reason` carries the value from the last assistant response before the loop ended. To detect refusals, check `stop_reason === "refusal"` (TypeScript) or `stop_reason == "refusal"` (Python). See [`SDKResultMessage`](./developer-agent-sdk-typescript.md#sdk-result-message) (TypeScript) or [`ResultMessage`](./developer-agent-sdk-python.md#result-message) (Python) for the full type.

## Hooks

[Hooks](./developer-agent-sdk-hooks.md) are callbacks that fire at specific points in the loop: before a tool runs, after it returns, when the agent finishes, and so on. Some commonly used hooks are:
...
Both SDKs support all the events above. The TypeScript SDK includes additional events that Python does not yet support. See [Control execution with hooks](./developer-agent-sdk-hooks.md) for the complete event list, per-SDK availability, and the full callback API.
...
- **Haven't run an agent yet?** Start with the [quickstart](./developer-agent-sdk-quickstart.md) to get the SDK installed and see a full example running end to end.
- **Ready to hook into your project?** [Load CLAUDE.md, skills, and filesystem hooks](./developer-agent-sdk-claude-code-features.md) so the agent follows your project conventions automatically.
- **Building an interactive UI?** Enable [streaming](./developer-agent-sdk-streaming-output.md) to show live text and tool calls as the loop runs.
- **Need tighter control over what the agent can do?** Lock down tool access with [permissions](./developer-agent-sdk-permissions.md), and use [hooks](./developer-agent-sdk-hooks.md) to audit, block, or transform tool calls before they execute.
- **Running long or expensive tasks?** Offload isolated work to [subagents](./developer-agent-sdk-subagents.md) to keep your main context lean.
