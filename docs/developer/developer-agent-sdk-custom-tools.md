...
|:-------------------|:--------|
| Define a tool | Use [`@tool`](./developer-agent-sdk-python.md#tool) (Python) or [`tool()`](./developer-agent-sdk-typescript.md#tool) (TypeScript) with a name, description, schema, and handler. See [Create a custom tool](#create-a-custom-tool). |
...
| Scale to many tools | Use [tool search](./developer-agent-sdk-tool-search.md) to load tools on demand. |

## Create a custom tool

A tool is defined by four parts, passed as arguments to the [`tool()`](./developer-agent-sdk-typescript.md#tool) helper in TypeScript or the [`@tool`](./developer-agent-sdk-python.md#tool) decorator in Python:
...
After defining a tool, wrap it in a server with [`createSdkMcpServer`](./developer-agent-sdk-typescript.md#create-sdk-mcp-server) (TypeScript) or [`create_sdk_mcp_server`](./developer-agent-sdk-python.md#create-sdk-mcp-server) (Python). The server runs in-process inside your application, not as a separate process.
...
See the [`tool()`](./developer-agent-sdk-typescript.md#tool) TypeScript reference or the [`@tool`](./developer-agent-sdk-python.md#tool) Python reference for full parameter details, including JSON Schema input formats and return value structure.
...
Every tool in this array consumes context window space on every turn. If you're defining dozens of tools, see [tool search](./developer-agent-sdk-tool-search.md) to load them on demand instead.
...
See `ToolAnnotations` in the [TypeScript](./developer-agent-sdk-typescript.md#tool-annotations) or [Python](./developer-agent-sdk-python.md#tool-annotations) reference.
...
| allowed tools | Permission | Listed tools run without a permission prompt. Unlisted tools remain available; calls go through the [permission flow](./developer-agent-sdk-permissions.md). |
...
To limit which built-ins Claude can use, prefer `tools` over disallowed tools. Omitting a tool from `tools` removes it from context so Claude never attempts it; listing it in `disallowedTools` (Python: `disallowed_tools`) blocks the call but leaves the tool visible, so Claude may waste a turn trying it. See [Configure permissions](./developer-agent-sdk-permissions.md) for the full evaluation order.
...
- If your server grows to dozens of tools, see [tool search](./developer-agent-sdk-tool-search.md) to defer loading them until Claude needs them.
- To connect to external MCP servers (filesystem, GitHub, Slack) instead of building your own, see [Connect MCP servers](./developer-agent-sdk-mcp.md).
- To control which tools run automatically versus requiring approval, see [Configure permissions](./developer-agent-sdk-permissions.md).

## Related documentation

- [TypeScript SDK Reference](./developer-agent-sdk-typescript.md)
- [Python SDK Reference](./developer-agent-sdk-python.md)
- [MCP Documentation](https://modelcontextprotocol.io)
- [SDK Overview](./developer-agent-sdk-overview.md)