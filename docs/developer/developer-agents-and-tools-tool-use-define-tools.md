...
For the full set of optional properties available on any tool definition, including `cache_control`, `strict`, `defer_loading`, and `allowed_callers`, see the [Tool reference](./developer-agents-and-tools-tool-use-tool-reference.md#tool-definition-properties).
...
Combine `tool_choice: {"type": "any"}` with [strict tool use](./developer-agents-and-tools-tool-use-strict-tool-use.md) to guarantee both that one of your tools will be called AND that the tool inputs strictly follow your schema. Set `strict: true` on your tool definitions to enable schema validation.
</Tip>
...
<CardGroup>
  <Card href="./developer-agents-and-tools-tool-use-handle-tool-calls.md" title="Handle tool calls">
    Parse tool_use blocks and format tool_result responses.
  </Card>
  <Card href="./developer-agents-and-tools-tool-use-tool-runner.md" title="Tool Runner (SDK)">
