...
1. **`query`**: the main entry point that creates the agentic loop. It returns an async iterator, so you use `async for` to stream messages as Claude works. See the full API in the [Python](./developer-agent-sdk-python.md#query) or [TypeScript](./developer-agent-sdk-typescript.md#query) SDK reference.

2. **`prompt`**: what you want Claude to do. Claude figures out which tools to use based on the task.

3. **`options`**: configuration for the agent. This example uses `allowedTools` to restrict Claude to `Read`, `Edit`, and `Glob`, and `permissionMode: "acceptEdits"` to auto-approve file changes. Other options include `systemPrompt`, `mcpServers`, and more. See all options for [Python](./developer-agent-sdk-python.md#claudeagentoptions) or [TypeScript](./developer-agent-sdk-typescript.md#claudeagentoptions).
...
<Note>
This example uses streaming to show progress in real-time. If you don't need live output (e.g., for background jobs or CI pipelines), you can collect all messages at once. See [Streaming vs. single-turn mode](./developer-agent-sdk-streaming-vs-single-mode.md) for details.
</Note>
...
- **[Permissions](./developer-agent-sdk-permissions.md)**: control what your agent can do and when it needs approval
- **[Hooks](./developer-agent-sdk-hooks.md)**: run custom code before or after tool calls
- **[Sessions](./developer-agent-sdk-sessions.md)**: build multi-turn agents that maintain context
- **[MCP servers](./developer-agent-sdk-mcp.md)**: connect to databases, browsers, APIs, and other external systems
- **[Hosting](./developer-agent-sdk-hosting.md)**: deploy agents to Docker, cloud, and CI/CD
