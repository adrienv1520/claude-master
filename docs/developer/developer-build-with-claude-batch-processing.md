...
The Message Batches API is a powerful, cost-effective way to asynchronously process large volumes of [Messages](../api/api-messages.md) requests. This approach is well-suited to tasks that do not require immediate responses, with most batches finishing in less than 1 hour while reducing costs by 50% and increasing throughput.
...
- Rate limits apply to both Batches API HTTP requests and the number of requests within a batch waiting to be processed. See [Message Batches API rate limits](../api/api-rate-limits.md#message-batches-api). Additionally, we may slow down processing based on current demand and your request volume. In that case, you may see more requests expiring after 24 hours.
...
All [active models](./developer-about-claude-models-overview.md) support the Message Batches API.
...
<Tip>
Since batches can take longer than 5 minutes to process, consider using the [1-hour cache duration](./developer-build-with-claude-prompt-caching.md#1-hour-cache-duration) with prompt caching for better cache hit rates when processing batches with shared context.
</Tip>
...
- A `params` object with the standard [Messages API](../api/api-messages.md) parameters
...
Validation of the `params` object for each message request is performed asynchronously, and validation errors are returned when processing of the entire batch has ended. You can ensure that you are building your input correctly by verifying your request shape with the [Messages API](../api/api-messages.md) first.
</Tip>
...
You can list all Message Batches in your Workspace using the [list endpoint](https://platform.claude.com/docs/en/api/listing-message-batches.md). The API supports pagination, automatically fetching additional pages as needed:
...
If your result has an error, its `result.error` will be set to our standard [error shape](../api/api-errors.md#error-shapes).
...
## FAQ
  ...
  <section title="Are there Message Batches API rate limits and do they interact with the Messages API rate limits?">
    ...
    The Message Batches API has HTTP requests-based rate limits in addition to limits on the number of requests in need of processing. See [Message Batches API rate limits](../api/api-rate-limits.md#message-batches-api). Usage of the Batches API does not affect rate limits in the Messages API.
