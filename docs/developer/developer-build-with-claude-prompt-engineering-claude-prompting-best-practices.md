...
<Tip>
  For an overview of model capabilities, see the [models overview](./developer-about-claude-models-overview.md). For details on what's new in Claude 4.6, see [What's new in Claude 4.6](./developer-about-claude-models-whats-new-claude-4-6.md). For migration guidance, see the [Migration guide](./developer-about-claude-models-migration-guide.md).
</Tip>
...
If you are migrating from [extended thinking](./developer-build-with-claude-extended-thinking.md) with `budget_tokens`, replace your thinking configuration and move budget control to `effort`:
...
<Info>
  For more information on thinking capabilities, see [Extended thinking](./developer-build-with-claude-extended-thinking.md) and [Adaptive thinking](./developer-build-with-claude-adaptive-thinking.md).
</Info>
...
Claude 4.6 and Claude 4.5 models feature [context awareness](./developer-build-with-claude-context-windows.md#context-awareness-in-claude-sonnet-4-6-sonnet-4-5-and-haiku-4-5), enabling the model to track its remaining context window (i.e. "token budget") throughout a conversation. This enables Claude to execute tasks and manage context more effectively by understanding how much space it has to work.
...
The [memory tool](./developer-agents-and-tools-tool-use-memory-tool.md) pairs naturally with context awareness for seamless context transitions.
...
One technique that has proven effective to further boost performance is to give Claude a crop tool or [skill](./developer-agents-and-tools-agent-skills-overview.md). Testing has shown consistent uplift on image evaluations when Claude is able to "zoom" in on relevant regions of an image. Anthropic has created a [cookbook for the crop tool](https://platform.claude.com/cookbook/multimodal-crop-tool).
...
4. **Update thinking configuration**: Claude 4.6 models use [adaptive thinking](./developer-build-with-claude-adaptive-thinking.md) (`thinking: {type: "adaptive"}`) instead of manual thinking with `budget_tokens`. Use the [effort parameter](./developer-build-with-claude-effort.md) to control thinking depth.
...
For detailed migration steps, see the [Migration guide](./developer-about-claude-models-migration-guide.md).
...
The extended thinking paths above use `budget_tokens` for predictable token usage. If your workload fits one of the following patterns, consider trying [adaptive thinking](./developer-build-with-claude-adaptive-thinking.md) instead:
