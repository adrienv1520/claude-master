...
- [Stream responses in real-time](./developer-agent-sdk-streaming-output.md): access raw API events including `message_delta` as they arrive
- [Structured outputs](./developer-agent-sdk-structured-outputs.md): get typed JSON responses from the agent
- [Tracking costs and usage](./developer-agent-sdk-cost-tracking.md): understand token usage and billing from result messages