...
- Rate limits apply to both Batches API HTTP requests and the number of requests within a batch waiting to be processed. See [Message Batches API rate limits](../api/rate-limits.md#message-batches-api). Additionally, we may slow down processing based on current demand and your request volume. In that case, you may see more requests expiring after 24 hours.
...
If your result has an error, its `result.error` will be set to our standard [error shape](../api/errors.md#error-shapes).
...
## FAQ
  ...
  <section title="Are there Message Batches API rate limits and do they interact with the Messages API rate limits?">
    ...
    The Message Batches API has HTTP requests-based rate limits in addition to limits on the number of requests in need of processing. See [Message Batches API rate limits](../api/rate-limits.md#message-batches-api). Usage of the Batches API does not affect rate limits in the Messages API.
