...
<Tip>
You can also inject context directly via `systemPrompt` without using CLAUDE.md files. See [Modify system prompts](./developer-agent-sdk-modifying-system-prompts.md). Use CLAUDE.md when you want the same context shared between interactive Claude Code sessions and your SDK agents.
</Tip>
...
<Note>
Skills must be created as filesystem artifacts (`.claude/skills/<name>/SKILL.md`). The SDK does not have a programmatic API for registering skills. See [Agent Skills in the SDK](./developer-agent-sdk-skills.md) for full details.
</Note>

For more on creating and using skills, see [Agent Skills in the SDK](./developer-agent-sdk-skills.md).
...
- **Programmatic hooks:** callback functions passed directly to `query()`. These run in your application process and can return structured decisions. See [Control execution with hooks](./developer-agent-sdk-hooks.md).
...
Hook callbacks receive the tool input and return a decision dict. Returning `{}` (an empty dict) means allow the tool to proceed. Returning `{"decision": "block", "reason": "..."}` prevents execution and the reason is sent to Claude as the tool result. See the [hooks guide](./developer-agent-sdk-hooks.md) for the full callback signature and return types.
...
<Note>
The TypeScript SDK supports additional hook events beyond Python, including `SessionStart`, `SessionEnd`, `TeammateIdle`, and `TaskCompleted`. See the [hooks guide](./developer-agent-sdk-hooks.md) for the full event compatibility table.
</Note>

For full details on programmatic hooks, see [Control execution with hooks](./developer-agent-sdk-hooks.md). For filesystem hook syntax, see [Hooks](https://code.claude.com/docs/en/hooks).
...
| Give the agent reference material it loads when relevant | [Skills](./developer-agent-sdk-skills.md) | `settingSources` + `allowedTools: ["Skill"]` |
| Run a reusable workflow (deploy, review, release) | [User-invocable skills](./developer-agent-sdk-skills.md) | `settingSources` + `allowedTools: ["Skill"]` |
| Delegate an isolated subtask to a fresh context (research, review) | [Subagents](./developer-agent-sdk-subagents.md) | `agents` parameter + `allowedTools: ["Agent"]` |
...
| Run deterministic logic on tool calls (audit, block, transform) | [Hooks](./developer-agent-sdk-hooks.md) | `hooks` parameter with callbacks, or shell scripts loaded via `settingSources` |
| Give Claude structured tool access to an external service | [MCP](./developer-agent-sdk-mcp.md) | `mcpServers` parameter |

<Tip>
**Subagents versus agent teams:** Subagents are ephemeral and isolated: fresh conversation, one task, summary returned to parent. Agent teams coordinate multiple independent Claude Code instances that share a task list and message each other directly. Agent teams are a CLI feature. See [What subagents inherit](./developer-agent-sdk-subagents.md#what-subagents-inherit) and the [agent teams comparison](https://code.claude.com/docs/en/agent-teams#compare-with-subagents) for details.
</Tip>
...
- [Skills in the SDK](./developer-agent-sdk-skills.md): Full guide to using skills programmatically
- [Subagents](./developer-agent-sdk-subagents.md): Define and invoke subagents for isolated subtasks
- [Hooks](./developer-agent-sdk-hooks.md): Intercept and control agent behavior at key execution points
- [Permissions](./developer-agent-sdk-permissions.md): Control tool access with modes, rules, and callbacks
- [System prompts](./developer-agent-sdk-modifying-system-prompts.md): Inject context without CLAUDE.md files