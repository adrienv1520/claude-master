...
<Note>
[Server tool](./agents-and-tools-tool-use-overview.md#server-tools) token counts only apply to the first sampling call.
</Note>
...
<Note>
See [here](./build-with-claude-extended-thinking.md#how-context-window-is-calculated-with-extended-thinking) for more details about how the context window is calculated with extended thinking
...
<Note>
Token counting supports PDFs with the same [limitations](./build-with-claude-pdf-support.md#pdf-support-limitations) as the Messages API.
</Note> 
...
Token counting is **free to use** but subject to requests per minute rate limits based on your [usage tier](../api/rate-limits.md#rate-limits). If you need higher limits, contact sales through the [Claude Console](/settings/limits).
