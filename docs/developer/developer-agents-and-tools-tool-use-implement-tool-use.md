...
<Tip>
If using Claude with tool use and extended thinking, refer to the [extended thinking guide](./developer-build-with-claude-extended-thinking.md) for more information.
</Tip>
...
The tool runner supports automatic [compaction](./developer-build-with-claude-context-editing.md#client-side-compaction-sdk), which generates summaries when token usage exceeds a threshold. This allows long-running agentic tasks to continue beyond context window limits.
</Tip>
...
You can modify tool results before they're sent back to Claude. This is useful for adding metadata like `cache_control` to enable [prompt caching](./developer-build-with-claude-prompt-caching.md) on tool results, or for transforming the tool output.
...
<Tip>
Adding `cache_control` to tool results is particularly useful when tools return large amounts of data (like document search results) that you want to cache for subsequent API calls. See [Prompt caching](./developer-build-with-claude-prompt-caching.md) for more details on caching strategies.
</Tip>
...
<Note>
When using [prompt caching](./developer-build-with-claude-prompt-caching.md#what-invalidates-the-cache), changes to the `tool_choice` parameter will invalidate cached message blocks. Tool definitions and system prompts remain cached, but message content must be reprocessed.
</Note>
...
<Note>
When using [extended thinking](./developer-build-with-claude-extended-thinking.md) with tool use, `tool_choice: {"type": "any"}` and `tool_choice: {"type": "tool", "name": "..."}` are not supported and will result in an error. Only `tool_choice: {"type": "auto"}` (the default) and `tool_choice: {"type": "none"}` are compatible with extended thinking.
</Note>
...
Combine `tool_choice: {"type": "any"}` with [strict tool use](./developer-build-with-claude-structured-outputs.md) to guarantee both that one of your tools will be called AND that the tool inputs strictly follow your schema. Set `strict: true` on your tool definitions to enable schema validation.
</Tip>

### JSON output

Tools do not necessarily need to be client functions. You can use tools anytime you want the model to return JSON output that follows a provided schema. For example, you might use a `record_summary` tool with a particular schema. See [Tool use with Claude](./developer-agents-and-tools-tool-use-overview.md) for a full working example.
...
public class ParallelToolUse
{
    public static async Task Main()
    {
        ...
        var initialParams = new MessageCreateParams
        {
            Model = Model.ClaudeOpus4_6,
            MaxTokens = 1024,
            Tools = tools,
        ...
        var response = await client.Messages.Create(initialParams);

        var finalParams = new MessageCreateParams
        {
            Model = Model.ClaudeOpus4_6,
            MaxTokens = 1024,
            Tools = tools,
            ...
            [
                ...
                new()
                {
                    ...
                    {
                        new ContentBlockParam(new ToolResultBlockParam() { ToolUseID = "toolu_01", Content = "San Francisco: 68\u00b0F, partly cloudy" }),
                        new ContentBlockParam(new ToolResultBlockParam() { ToolUseID = "toolu_02", Content = "New York: 45\u00b0F, clear skies" }),
                        new ContentBlockParam(new ToolResultBlockParam() { ToolUseID = "toolu_03", Content = "San Francisco time: 2:30 PM PST" }),
                        new ContentBlockParam(new ToolResultBlockParam() { ToolUseID = "toolu_04", Content = "New York time: 5:30 PM EST" }),
                    }),
                }
            ]
        };

        var finalResponse = await client.Messages.Create(finalParams);
...
Claude Sonnet 3.7 may be less likely to make parallel tool calls in a response, even when you have not set `disable_parallel_tool_use`. [Upgrade to Claude 4 models](./developer-about-claude-models-migration-guide.md), which have built-in token-efficient tool use and improved parallel tool calling.

If you're still using Claude Sonnet 3.7, you can enable the `token-efficient-tools-2025-02-19` [beta header](../api/api-beta-headers.md), which helps encourage Claude to use parallel tools. You can also introduce a "batch tool" that can act as a meta-tool to wrap invocations to other tools simultaneously.
...
If Claude's [response is cut off due to hitting the `max_tokens` limit](./developer-build-with-claude-handling-stop-reasons.md#max-tokens), and the truncated response contains an incomplete tool use block, you'll need to retry the request with a higher `max_tokens` value to get the full tool use.
...
<Tip>
To eliminate invalid tool calls entirely, use [strict tool use](./developer-build-with-claude-structured-outputs.md) with `strict: true` on your tool definitions. This guarantees that tool inputs will always match your schema exactly, preventing missing parameters and type mismatches.
</Tip>
...
- Claude Sonnet 3.7: May need stronger prompting or the `token-efficient-tools-2025-02-19` [beta header](../api/api-beta-headers.md). Consider [upgrading to Claude 4](./developer-about-claude-models-migration-guide.md).
