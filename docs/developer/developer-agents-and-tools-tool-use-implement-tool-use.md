...
<Info>
Tool use examples is a beta feature. Include the appropriate [beta header](../api/api-beta-headers.md) for your provider:
...
Claude Sonnet 3.7 may be less likely to make make parallel tool calls in a response, even when you have not set `disable_parallel_tool_use`. We recommend [upgrading to Claude 4 models](./developer-about-claude-models-migration-guide.md), which have built-in token-efficient tool use and improved parallel tool calling.
