...
- **Claude Sonnet 4.6** supports both [adaptive thinking](./developer-build-with-claude-adaptive-thinking.md) (where effort controls thinking depth) and manual thinking with [interleaved mode](./developer-build-with-claude-extended-thinking.md#interleaved-thinking) (`thinking: {type: "enabled", budget_tokens: N}`).
