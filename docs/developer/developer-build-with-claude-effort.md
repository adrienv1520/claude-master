...
- **Claude Sonnet 4.6** uses [adaptive thinking](./developer-build-with-claude-adaptive-thinking.md) (where effort controls thinking depth). Manual thinking with [interleaved mode](./developer-build-with-claude-extended-thinking.md#interleaved-thinking) (`thinking: {type: "enabled", budget_tokens: N}`) is still functional but deprecated.
