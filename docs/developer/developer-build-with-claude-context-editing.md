...
| **Client-side** | SDK | Compaction | Available in [Python and TypeScript SDKs](../api/api-client-sdks.md) when using [`tool_runner`](./developer-agents-and-tools-tool-use-implement-tool-use.md#tool-runner-beta). Generates a summary and replaces full conversation history. See [Compaction](#client-side-compaction-sdk) below. |
