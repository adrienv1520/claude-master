...
1. **Tool needs approval**: Claude wants to use a tool that isn't auto-approved by [permission rules](./developer-agent-sdk-permissions.md) or modes. Check `tool_name` for the tool (e.g., `"Bash"`, `"Write"`).
...
<Note>
To automatically allow or deny tools without prompting users, use [hooks](./developer-agent-sdk-hooks.md) instead. Hooks execute before `canUseTool` and can allow, deny, or modify requests based on your own logic. You can also use the [`PermissionRequest` hook](./developer-agent-sdk-hooks.md#available-hooks) to send external notifications (Slack, email, push) when Claude is waiting for approval.
</Note>
...
<Note>
In Python, `can_use_tool` requires [streaming mode](./developer-agent-sdk-streaming-vs-single-mode.md) and a `PreToolUse` hook that returns `{"continue_": True}` to keep the stream open. Without this hook, the stream closes before the permission callback can be invoked.
</Note>
...
- **Redirect entirely**: use [streaming input](./developer-agent-sdk-streaming-vs-single-mode.md) to send Claude a completely new instruction
...
<Tabs>
  ...
  <Tab title="Redirect entirely">
    For a complete change of direction (not just a nudge), use [streaming input](./developer-agent-sdk-streaming-vs-single-mode.md) to send Claude a new instruction directly. This bypasses the current tool request and gives Claude entirely new instructions to follow.
  </Tab>
</Tabs>
...
<Tip>
Clarifying questions are especially common in [`plan` mode](./developer-agent-sdk-permissions.md#plan-mode-plan), where Claude explores the codebase and asks questions before proposing a plan. This makes plan mode ideal for interactive workflows where you want Claude to gather requirements before making changes.
</Tip>
...
Use [streaming input](./developer-agent-sdk-streaming-vs-single-mode.md) when you need to:
...
Use [custom tools](./developer-agent-sdk-custom-tools.md) when you need to:
