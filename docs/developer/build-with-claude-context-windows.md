...
When using [extended thinking](./build-with-claude-extended-thinking.md), all input and output tokens, including the tokens used for thinking, count toward the context window limit, with a few nuances in multi-turn situations.
...
<Note>
You can read more about the context window and extended thinking in our [extended thinking guide](./build-with-claude-extended-thinking.md).
</Note>
...
<Note>
Claude 4 models support [interleaved thinking](./build-with-claude-extended-thinking.md#interleaved-thinking), which enables Claude to think between tool calls and make more sophisticated reasoning after receiving tool results.
...
For more information about using tools with extended thinking, see our [extended thinking guide](./build-with-claude-extended-thinking.md#extended-thinking-with-tool-use).
</Note>
...
- **Pricing**: Requests exceeding 200K tokens are automatically charged at premium rates (2x input, 1.5x output pricing). See the [pricing documentation](./about-claude-pricing.md#long-context-pricing) for details.
- **Rate limits**: Long context requests have dedicated rate limits. See the [rate limits documentation](../api/rate-limits.md#long-context-rate-limits) for details.
- **Multimodal considerations**: When processing large numbers of images or pdfs, be aware that the files can vary in token usage. When pairing a large prompt with a large number of images, you may hit [request size limits](../api/overview.md#request-size-limits).
...
For prompting guidance on leveraging context awareness, see our [Claude 4 best practices guide](./build-with-claude-prompt-engineering-claude-4-best-practices.md#context-awareness-and-multi-window-workflows).
...
To plan your token usage and ensure you stay within context window limits, you can use the [token counting API](./build-with-claude-token-counting.md) to estimate how many tokens your messages will use before sending them to Claude.

See our [model comparison](./about-claude-models-overview.md#model-comparison-table) table for a list of context window sizes by model.

# Next steps
<CardGroup cols={2}>
  <Card title="Model comparison table" icon="scales" href="./about-claude-models-overview.md">
