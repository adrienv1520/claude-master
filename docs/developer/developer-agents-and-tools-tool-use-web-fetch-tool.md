...
<Note>
The basic web fetch tool (`web_fetch_20250910`) is eligible for [Zero Data Retention (ZDR)](./developer-build-with-claude-zero-data-retention.md).
...
<Note>
Dynamic filtering requires the [code execution tool](./developer-agents-and-tools-tool-use-code-execution-tool.md) to be enabled. The web fetch tool (with and without dynamic filtering) is available on the Claude API and Microsoft Azure.
</Note>
...
Web fetch works with [prompt caching](./developer-build-with-claude-prompt-caching.md). To enable prompt caching, add `cache_control` breakpoints in your request. Cached fetch results can be reused across conversation turns.
...
You can include the web fetch tool in the [Messages Batches API](./developer-build-with-claude-batch-processing.md). Web fetch tool calls through the Messages Batches API are priced the same as those in regular Messages API requests.
