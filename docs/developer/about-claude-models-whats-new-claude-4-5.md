...
Claude Opus 4.5 is the only model that supports the [effort parameter](./build-with-claude-effort.md), allowing you to control how many tokens Claude uses when responding. This gives you the ability to trade off between response thoroughness and token efficiency with a single model.
...
Claude Opus 4.5 introduces [enhanced computer use capabilities](./agents-and-tools-tool-use-computer-use-tool.md) with a new zoom action that enables detailed inspection of specific screen regions at full resolution. This allows Claude to examine fine-grained UI elements, small text, and detailed visual information that might be unclear in standard screenshots.
...
Claude Opus 4.5 delivers flagship intelligence at a [more accessible price point](./about-claude-pricing.md) than previous Opus models, making advanced AI capabilities available for a broader range of applications and use cases.

### Thinking block preservation

Claude Opus 4.5 [automatically preserves all previous thinking blocks](./build-with-claude-extended-thinking.md#thinking-block-preservation-in-claude-opus-4-5) throughout conversations, maintaining reasoning continuity across extended multi-turn interactions and tool use sessions. This ensures Claude can effectively leverage its full reasoning history when working on complex, long-running tasks.
...
<Note>
Claude Sonnet 4.5 performs significantly better on coding tasks when [extended thinking](./build-with-claude-extended-thinking.md) is enabled. Extended thinking is disabled by default, but we recommend enabling it for complex coding work. Be aware that extended thinking impacts [prompt caching efficiency](./build-with-claude-prompt-caching.md#caching-with-thinking-blocks). See the [migration guide](./about-claude-models-migrating-to-claude-4.md#extended-thinking-recommendations) for configuration details.
</Note>
...
For detailed guidance on working with this communication style, see [Claude 4 best practices](./build-with-claude-prompt-engineering-claude-4-best-practices.md).
...
Extended thinking must be enabled explicitly by adding a `thinking` parameter to your API requests. See the [Extended thinking documentation](./build-with-claude-extended-thinking.md) for implementation details.

<Note>
Claude Haiku 4.5 performs significantly better on coding and reasoning tasks when [extended thinking](./build-with-claude-extended-thinking.md) is enabled. Extended thinking is disabled by default, but we recommend enabling it for complex problem-solving, coding work, and multi-step reasoning. Be aware that extended thinking impacts [prompt caching efficiency](./build-with-claude-prompt-caching.md#caching-with-thinking-blocks). See the [migration guide](./about-claude-models-migrating-to-claude-4.md#extended-thinking-recommendations) for configuration details.
</Note>
...
This is the first Haiku model with native context awareness capabilities. For prompting guidance, see [Claude 4 best practices](./build-with-claude-prompt-engineering-claude-4-best-practices.md#context-awareness-and-multi-window-workflows).
...
<Note>Available in Claude Opus 4.5 and Claude Sonnet 4.5. Requires [beta header](../api/beta-headers.md): `advanced-tool-use-2025-11-20`</Note>

### Tool search tool (Beta)

The [tool search tool](./agents-and-tools-tool-use-tool-search-tool.md) enables Claude to work with hundreds or thousands of tools by dynamically discovering and loading them on-demand. Instead of loading all tool definitions into the context window upfront, Claude searches your tool catalog and loads only the tools it needs.
...
The [effort parameter](./build-with-claude-effort.md) allows you to control how many tokens Claude uses when responding, trading off between response thoroughness and token efficiency:
...
<Note>Available exclusively in Claude Opus 4.5. Requires [beta header](../api/beta-headers.md): `effort-2025-11-24`</Note>

### Tool use examples (Beta)

[Tool use examples](./agents-and-tools-tool-use-implement-tool-use.md#providing-tool-use-examples) allow you to provide concrete examples of valid tool inputs to help Claude understand how to use your tools more effectively. This is particularly useful for complex tools with nested objects, optional parameters, or format-sensitive inputs.
...
<Note>Available in Claude Sonnet 4.5, Haiku 4.5, Opus 4.5, Opus 4.1, and Opus 4. Requires [beta header](../api/beta-headers.md): `advanced-tool-use-2025-11-20`.</Note>

### Memory tool (Beta)

The new [memory tool](./agents-and-tools-tool-use-memory-tool.md) enables Claude to store and retrieve information outside the context window:
...
<Note>Available in Claude Sonnet 4, Sonnet 4.5, Haiku 4.5, Opus 4, Opus 4.1, and Opus 4.5. Requires [beta header](../api/beta-headers.md): `context-management-2025-06-27`</Note>

### Context editing

Use [context editing](./build-with-claude-context-editing.md) for intelligent context management through automatic tool call clearing:
...
When using Claude 4 models, you should update your application to [handle `refusal` stop reasons](./test-and-evaluate-strengthen-guardrails-handle-streaming-refusals.md).
...
For more information, see the [Extended thinking documentation](./build-with-claude-extended-thinking.md#summarized-thinking).
...
<Note>
Interleaved thinking is in beta. To enable interleaved thinking, add [the beta header](../api/beta-headers.md) `interleaved-thinking-2025-05-14` to your API request.
</Note>

For more information, see the [Extended thinking documentation](./build-with-claude-extended-thinking.md#interleaved-thinking).
...
For comprehensive guidance on working with these models, see [Claude 4 prompt engineering best practices](./build-with-claude-prompt-engineering-claude-4-best-practices.md).
...
For more information, see the [Text editor tool documentation](./agents-and-tools-tool-use-text-editor-tool.md).
...
For migration instructions, see the [Code execution tool documentation](./agents-and-tools-tool-use-code-execution-tool.md#upgrade-to-latest-tool-version).
...
For more details, see the [pricing documentation](./about-claude-pricing.md).
...
- [AWS Bedrock global vs regional endpoints](./build-with-claude-claude-on-amazon-bedrock.md#global-vs-regional-endpoints)
- [Google Vertex AI global vs regional endpoints](./build-with-claude-claude-on-vertex-ai.md#global-vs-regional-endpoints)
...
Breaking changes and migration requirements vary depending on which model you're upgrading from. For detailed migration instructions, including step-by-step guides, breaking changes, and migration checklists, see [Migrating to Claude 4.5](./about-claude-models-migrating-to-claude-4.md).
...
<CardGroup cols={3}>
  <Card title="Best practices" icon="lightbulb" href="./build-with-claude-prompt-engineering-claude-4-best-practices.md">
    Learn prompt engineering techniques for Claude 4.5 models
  </Card>
  <Card title="Model overview" icon="table" href="./about-claude-models-overview.md">
    Compare Claude 4.5 models with other Claude models
  </Card>
  <Card title="Migration guide" icon="arrow-right-arrow-left" href="./about-claude-models-migrating-to-claude-4.md">
