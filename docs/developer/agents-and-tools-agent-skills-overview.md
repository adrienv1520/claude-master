...
The [Claude Agent SDK](./agent-sdk-overview.md) supports custom Skills through filesystem-based configuration.
...
To learn more, see [Agent Skills in the SDK](./agent-sdk-skills.md).
...
These Skills are available on the Claude API and claude.ai. See the [quickstart tutorial](./agents-and-tools-agent-skills-quickstart.md) to start using them in the API.
...
- **Claude API**:
    ...
    - **Pre-configured dependencies only**: Check the [code execution tool documentation](./agents-and-tools-tool-use-code-execution-tool.md) for the list of available packages
- **Claude Code**:
