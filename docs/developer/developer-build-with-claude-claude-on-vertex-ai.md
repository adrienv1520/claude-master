...
The Vertex API for accessing Claude is nearly-identical to the [Messages API](../api/api-messages.md) and supports all of the same options, with two key differences:

* In Vertex, `model` is not passed in the request body. Instead, it is specified in the Google Cloud endpoint URL.
* In Vertex, `anthropic_version` is passed in the request body (rather than as a header), and must be set to the value `vertex-2023-10-16`.

Vertex is also supported by Anthropic's official [client SDKs](../api/api-client-sdks.md). This guide will walk you through the process of making a request to Claude on Vertex AI in either Python or TypeScript.
...
<Note>
The PHP SDK does not currently support Google Vertex AI. For available SDK platform integrations, see [Client SDKs](../api/api-client-sdks.md).
</Note>
...
Claude is also available through [Amazon Bedrock](./developer-build-with-claude-claude-on-amazon-bedrock.md) and [Microsoft Foundry](./developer-build-with-claude-claude-in-microsoft-foundry.md).
...
## Feature support
For all currently supported features on Vertex AI, see [API features overview](../api/api-overview.md).
