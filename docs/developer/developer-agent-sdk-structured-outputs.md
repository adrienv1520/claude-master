...
- [API Structured Outputs](./developer-build-with-claude-structured-outputs.md): use structured outputs with the Claude API directly for single-turn requests without tool use
- [Custom tools](./developer-agent-sdk-custom-tools.md): give your agent custom tools to call during execution before returning structured output