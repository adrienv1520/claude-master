...
Claude Opus 4.6 is a near drop-in replacement for Claude 4.5, with a few breaking changes to be aware of. For a full list of new features, see [What's new in Claude 4.6](./developer-about-claude-models-whats-new-claude-4-6.md).
...
1. **Prefill removal:** Prefilling assistant messages returns a 400 error on Claude 4.6 models. Use [structured outputs](./developer-build-with-claude-structured-outputs.md), system prompt instructions, or `output_config.format` instead.
...
2. **Update tool versions**
   ...
   - **Text editor:** Use `text_editor_20250728` and `str_replace_based_edit_tool`. See [Text editor tool documentation](./developer-agents-and-tools-tool-use-text-editor-tool.md) for details.
   - **Code execution:** Upgrade to `code_execution_20250825`. See [Code execution tool documentation](./developer-agents-and-tools-tool-use-code-execution-tool.md#upgrade-to-latest-tool-version) for migration instructions.

3. **Handle the `refusal` stop reason**

   Update your application to [handle `refusal` stop reasons](./developer-test-and-evaluate-strengthen-guardrails-handle-streaming-refusals.md):
...
6. **Update your prompts for behavioral changes**
   ...
   Claude 4+ models have a more concise, direct communication style and require explicit direction. Review [prompting best practices](./developer-build-with-claude-prompt-engineering-claude-prompting-best-practices.md) for optimization guidance.
...
- [ ] **Recommended:** Migrate from `thinking: {type: "enabled", budget_tokens: N}` to `thinking: {type: "adaptive"}` with the [effort parameter](./developer-build-with-claude-effort.md) (`budget_tokens` is deprecated and will be removed in a future release)
...
- [ ] Review and update prompts following [prompting best practices](./developer-build-with-claude-prompt-engineering-claude-prompting-best-practices.md)
...
For a complete overview of capabilities, see the [models overview](./developer-about-claude-models-overview.md).

<Note>
Sonnet 4.6 pricing is $3 per million input tokens, $15 per million output tokens. See [Claude pricing](./developer-about-claude-pricing.md) for details.
</Note>
...
1. **Prefilling assistant messages is no longer supported**
   ...
   Prefilling assistant messages returns a `400` error on Sonnet 4.6. Use [structured outputs](./developer-build-with-claude-structured-outputs.md), system prompt instructions, or `output_config.format` instead.

   **Common prefill use cases and migrations:**

   - **Controlling output formatting** (forcing JSON/YAML output): Use [structured outputs](./developer-build-with-claude-structured-outputs.md) or tools with enum fields for classification tasks.
...
6. **Update your prompts for behavioral changes**
   ...
   Claude 4 models have a more concise, direct communication style. Review [prompting best practices](./developer-build-with-claude-prompt-engineering-claude-prompting-best-practices.md) for optimization guidance.
...
If you're using extended thinking with `budget_tokens` on Sonnet 4.5, it is still functional on Sonnet 4.6 but is deprecated. Migrate to [adaptive thinking](./developer-build-with-claude-adaptive-thinking.md) with the [effort parameter](./developer-build-with-claude-effort.md).

##### Migrating to adaptive thinking

[Adaptive thinking](./developer-build-with-claude-adaptive-thinking.md) is the recommended replacement for `budget_tokens` on Sonnet 4.6. It is particularly well suited to the following workload patterns:
...
<Note>
If you see inconsistent behavior or quality regressions with adaptive thinking, try lowering the [effort](./developer-build-with-claude-effort.md) setting or using `max_tokens` as a hard limit first. Extended thinking with `budget_tokens` is still functional on Sonnet 4.6 but is deprecated and no longer recommended.
</Note>
...
<Note>
Sonnet 4.5 pricing is $3 per million input tokens, $15 per million output tokens. See [Claude pricing](./developer-about-claude-pricing.md) for details.
</Note>
...
<Note>
Haiku 4.5 pricing is $1 per million input tokens, $5 per million output tokens. See [Claude pricing](./developer-about-claude-pricing.md) for details.
</Note>
...
**Review new rate limits:** Haiku 4.5 has separate rate limits from Haiku 3.5 and Haiku 3. See [Rate limits documentation](../api/api-rate-limits.md) for details.
...
<Note>
Extended thinking impacts [prompt caching](./developer-build-with-claude-prompt-caching.md#caching-with-thinking-blocks) efficiency.

Extended thinking is deprecated in Claude 4.6 or newer models. If using newer models, use [adaptive thinking](./developer-build-with-claude-adaptive-thinking.md) instead.
</Note>

**Explore new capabilities:** See the [models overview](./developer-about-claude-models-overview.md) for details on context awareness, increased output capacity (64k tokens), higher intelligence, and improved speed.
...
- Check the [API documentation](../api/api-overview.md) for detailed specifications
- Review [model capabilities](./developer-about-claude-models-overview.md) for performance comparisons
- Review [API release notes](https://platform.claude.com/docs/en/release-notes/api.md) for API updates
