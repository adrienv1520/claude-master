...
<Warning>
`budget_tokens` is deprecated on Claude Opus 4.6 and will be removed in a future model release. We recommend using [adaptive thinking](./developer-build-with-claude-adaptive-thinking.md) with the [effort parameter](./developer-build-with-claude-effort.md) to control thinking depth instead.
</Warning>
...
2. **Context maintenance**: While tool results appear as user messages in the API structure, they're part of a continuous reasoning flow. Preserving thinking blocks maintains this conceptual flow across multiple API calls. For more information on context management, see our [guide on context windows](./developer-build-with-claude-context-windows.md).
...
<Note>
You can read through our [guide on context windows](./developer-build-with-claude-context-windows.md) for a more thorough deep dive.
</Note>
...
We recommend using the [token counting API](./developer-build-with-claude-token-counting.md) to get accurate token counts for your specific use case, especially when working with multi-turn conversations that include thinking.
...
- **Large budgets:** For thinking budgets above 32k, we recommend using [batch processing](./developer-build-with-claude-batch-processing.md) to avoid networking issues. Requests pushing the model to think above 32k tokens causes long running requests that might run up against system timeouts and open connection limits.
...
- **Prompt engineering:** Review our [extended thinking prompting tips](./developer-build-with-claude-prompt-engineering-extended-thinking-tips.md) if you want to maximize Claude's thinking capabilities.
