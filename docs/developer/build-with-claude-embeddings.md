...
Note that we use `input_type="document"` and `input_type="query"` for embedding the document and query, respectively. More specification can be found [here](./build-with-claude-embeddings.md#voyage-python-package).
