...
<Note>
**Try the new V2 interface (preview):** A simplified interface with `send()` and `stream()` patterns is now available, making multi-turn conversations easier. [Learn more about the TypeScript V2 preview](./developer-agent-sdk-typescript-v2-preview.md)
</Note>
...
| `allowedTools` | `string[]` | `[]` | Tools to auto-approve without prompting. This does not restrict Claude to only these tools; unlisted tools fall through to `permissionMode` and `canUseTool`. Use `disallowedTools` to block tools. See [Permissions](./developer-agent-sdk-permissions.md#allow-and-deny-rules) |
...
| `enableFileCheckpointing` | `boolean` | `false` | Enable file change tracking for rewinding. See [File checkpointing](./developer-agent-sdk-file-checkpointing.md) |
...
| `outputFormat` | `{ type: 'json_schema', schema: JSONSchema }` | `undefined` | Define output format for agent results. See [Structured outputs](./developer-agent-sdk-structured-outputs.md) for details |
...
| `plugins` | [`SdkPluginConfig`](#sdk-plugin-config)`[]` | `[]` | Load custom plugins from local paths. See [Plugins](./developer-agent-sdk-plugins.md) for details |
...
| `rewindFiles(userMessageId, options?)` | Restores files to their state at the specified user message. Pass `{ dryRun: true }` to preview changes. Requires `enableFileCheckpointing: true`. See [File checkpointing](./developer-agent-sdk-file-checkpointing.md) |
...
|:------|:-----|:------------|
| `askUserQuestion.previewFormat` | `'markdown' \| 'html'` | Opts into the `preview` field on [`AskUserQuestion`](./developer-agent-sdk-user-input.md#question-format) options and sets its content format. When unset, Claude does not emit previews |
...
For complete information on creating and using plugins, see [Plugins](./developer-agent-sdk-plugins.md).
...
The `message` field is a [`BetaMessage`](../api/api-messages.md) from the Anthropic SDK. It includes fields like `id`, `content`, `model`, `stop_reason`, and `usage`.
...
For a comprehensive guide on using hooks with examples and common patterns, see the [Hooks guide](./developer-agent-sdk-hooks.md).
...
Available beta features that can be enabled via the `betas` option. See [Beta headers](../api/api-beta-headers.md) for more information.
...
<Warning>
The `context-1m-2025-08-07` beta is retired as of April 30, 2026. Passing this value with Claude Sonnet 4.5 or Sonnet 4 has no effect, and requests that exceed the standard 200k-token context window return an error. To use a 1M-token context window, migrate to [Claude Sonnet 4.6 or Claude Opus 4.6](./developer-about-claude-models-overview.md), which include 1M context at standard pricing with no beta header required.
</Warning>
...
- [SDK overview](./developer-agent-sdk-overview.md) - General SDK concepts
- [Python SDK reference](./developer-agent-sdk-python.md) - Python SDK documentation
