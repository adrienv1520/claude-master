...
<Tip>
This page covers output streaming (receiving tokens in real-time). For input modes (how you send messages), see [Send messages to agents](./developer-agent-sdk-streaming-vs-single-mode.md). You can also [stream responses using the Agent SDK via the CLI](https://code.claude.com/docs/en/headless).
</Tip>
...
The `event` field contains the raw streaming event from the [Claude API](./developer-build-with-claude-streaming.md#event-types). Common event types include:
...
- **Structured output**: the JSON result appears only in the final `ResultMessage.structured_output`, not as streaming deltas. See [structured outputs](./developer-agent-sdk-structured-outputs.md) for details.

## Next steps

Now that you can stream text and tool calls in real-time, explore these related topics:

- [Interactive vs one-shot queries](./developer-agent-sdk-streaming-vs-single-mode.md): choose between input modes for your use case
- [Structured outputs](./developer-agent-sdk-structured-outputs.md): get typed JSON responses from the agent
- [Permissions](./developer-agent-sdk-permissions.md): control which tools the agent can use