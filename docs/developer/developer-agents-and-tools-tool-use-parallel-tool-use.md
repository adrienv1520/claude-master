...
This page covers parallel tool calls: when Claude calls multiple tools in one turn, how to format the message history so parallelism keeps working, and how to disable it. For the single-call flow, see [Handle tool calls](./developer-agents-and-tools-tool-use-handle-tool-calls.md).
...
<Note>
**Simpler with Tool Runner**: The example below shows manual parallel tool handling. For most use cases, [Tool Runner](./developer-agents-and-tools-tool-use-tool-runner.md) automatically handles parallel tool execution with much less code.
</Note>
...
See [Handle tool calls](./developer-agents-and-tools-tool-use-handle-tool-calls.md) for other formatting rules.
...
- For the single-tool-call flow and `tool_result` formatting rules, see [Handle tool calls](./developer-agents-and-tools-tool-use-handle-tool-calls.md).
- For the SDK abstraction that handles parallel execution automatically, see [Tool Runner](./developer-agents-and-tools-tool-use-tool-runner.md).
- For the full tool-use workflow, see [Define tools](./developer-agents-and-tools-tool-use-define-tools.md).