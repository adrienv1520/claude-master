...
This page covers the tool-call lifecycle: reading `tool_use` blocks from Claude's response, formatting `tool_result` blocks in your reply, and signaling errors. For the SDK abstraction that handles this automatically, see [Tool Runner](./developer-agents-and-tools-tool-use-tool-runner.md).

<Note>
**Simpler with Tool Runner**: The manual tool handling described on this page is automatically managed by [Tool Runner](./developer-agents-and-tools-tool-use-tool-runner.md). Use this page when you need custom control over tool execution.
</Note>
...
<Tip>
To eliminate invalid tool calls entirely, use [strict tool use](./developer-agents-and-tools-tool-use-strict-tool-use.md) with `strict: true` on your tool definitions. This guarantees that tool inputs will always match your schema exactly, preventing missing parameters and type mismatches.
</Tip>
...
- For running multiple tools in one turn, see [Parallel tool use](./developer-agents-and-tools-tool-use-parallel-tool-use.md).
- For the SDK abstraction that automates this loop, see [Tool Runner](./developer-agents-and-tools-tool-use-tool-runner.md).
