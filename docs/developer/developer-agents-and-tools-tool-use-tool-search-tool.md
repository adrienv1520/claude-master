...
<Note>
The tool search tool is currently in public beta. Include the appropriate [beta header](../api/api-beta-headers.md) for your provider:
...
The tool search tool works with [MCP servers](./developer-agents-and-tools-mcp-connector.md). Add the `"mcp-client-2025-11-20"` [beta header](../api/api-beta-headers.md) to your API request, and then use `mcp_toolset` with `default_config` to defer loading MCP tools:
...
<Note>
  ...
  examples](./developer-agents-and-tools-tool-use-implement-tool-use.md#providing-tool-use-examples).
...
Tool search works with [prompt caching](./developer-build-with-claude-prompt-caching.md). Add `cache_control` breakpoints to optimize multi-turn conversations:
...
You can include the tool search tool in the [Messages Batches API](./developer-build-with-claude-batch-processing.md). Tool search operations through the Messages Batches API are priced the same as those in regular Messages API requests.
